###############################################################
# $Id: 72_FB_FORK.pm 17437 2023-01-05 20:30:00Z fork $
#
#  Based on 72_FRITZBOX.pm 
#
...
#
##############################################################################
#  This is a fork of the original work of Torsten Poitzsch an tupol
#  It upgrades the module with additional functions of the Fritz!Box.
#
#  (c) jowiemann https://forum.fhem.de/index.php/topic,118150.0.html
##############################################################################
#
# define <name> FB_FORK
#
...
our $FB_FORK_TR064pwd;
our $FB_FORK_TR064user;
...
sub FB_FORK_Log($$$);
sub FB_FORK_Init($);
sub FB_FORK_Set_Cmd_Start($);
sub FB_FORK_Shell_Exec($$);
sub FB_FORK_StartRadio_Shell($@);
sub FB_FORK_StartRadio_Web($@);
sub FB_FORK_Readout_Add_Reading ($$$$@);
sub FB_FORK_Readout_Process($$);
sub FB_FORK_SendMail_Shell($@);
sub FB_FORK_SetCustomerRingTone($@);
sub FB_FORK_SetMOH($@);
sub FB_FORK_TR064_Init($$);
sub FB_FORK_Wlan_Run($);
sub FB_FORK_Web_Query($$@);
sub FB_FORK_Lua_Data($$@);
sub FB_FORK_Process_JSON($$$@);
sub FB_FORK_ERR_Result($$;@);
sub FB_FORK_Run_macFilter($);
sub FB_FORK_Run_chgProfile($);
sub FB_FORK_Run_lockLandevice($);
sub FB_FORK_Run_enableVPNshare($);
our $telnet;
...
sub FB_FORK_Log($$$)
{
   ...
   Log3 $hash, $loglevel, "FB_FORK [$instName: $sub.$xline] - " . $text;
} # End FB_FORK_Log

#######################################################################
sub FB_FORK_Initialize($)
{
  my ($hash) = @_;

  $hash->{DefFn}    = "FB_FORK_Define";
  $hash->{UndefFn}  = "FB_FORK_Undefine";
  $hash->{DeleteFn} = "FB_FORK_Delete";
  $hash->{RenameFn} = "FB_FORK_Rename";

  $hash->{SetFn}    = "FB_FORK_Set";
  $hash->{GetFn}    = "FB_FORK_Get";
  $hash->{AttrFn}   = "FB_FORK_Attr";
...
sub FB_FORK_Define($$)
{
   ...
   return "Usage: define <name> FB_FORK [IP address]" if(@args <2 || @args >3);  
...
# stop if certain perl moduls are missing
   if ( $missingModul ) {
      $msg = "ERROR: Cannot define a FB_FORK device. Perl modul $missingModul is missing.";
      FB_FORK_Log $hash, 1, $msg;
      return $msg;
   }
   ...
   # my $tr064Port = FB_FORK_TR064_Init ($hash);
 ...
 # Check APIs after fhem.cfg is processed
   ...
   InternalTimer(gettimeofday()+1 , "FB_FORK_Readout_Start", $hash->{helper}{TimerReadout}, 0);
...
sub FB_FORK_Rename($$)
{
    ...
    my $old_index = "FB_FORK_".$old."_passwd";
    my $new_index = "FB_FORK_".$new."_passwd";
...
sub FB_FORK_Attr($@)
{
   ...
   if ( $aName =~ /fritzBoxIP|m3uFileLocal|m3uFileURL/ && $hash->{APICHECKED} == 1 || $aName eq "disable" ) {
      $hash->{APICHECKED} = 0;
      RemoveInternalTimer($hash->{helper}{TimerReadout});
      InternalTimer(gettimeofday()+1, "FB_FORK_Readout_Start", $hash->{helper}{TimerReadout}, 1);
      # FB_FORK_Readout_Start($hash->{helper}{TimerReadout});
   }
...
# set alarm
   if ( lc $cmd eq 'alarm') {
      if ( int @val > 0 && $val[0] =~ /^(1|2|3)$/ ) {
         FB_FORK_Log $hash, 5, "DEBUG: set $name $cmd ".join(" ", @val);
         unless ($hash->{WEBCM}==1 || $forceShell) {
            FB_FORK_Log $hash, 2, "'set ... alarm' is not supported by the limited interfaces of your Fritz!Box firmware.";
            return "ERROR: 'set ... alarm' is not supported by the limited interfaces of your Fritz!Box firmware.";
         }

         return FB_FORK_Set_Alarm_Web ($hash, @val) unless $forceShell;
         return FB_FORK_Set_Alarm_Shell ($hash, @val);
      }
...
# set call
      ...
      if (int @val > 0) {
         ...
         return FB_FORK_Set_Cmd_Start $hash->{helper}{TimerCmd};
      }
   # } elsif ( lc $cmd eq 'convertmoh') {
      # if (int @val > 0) 
      # {
         # FB_FORK_Log $hash, 5, "DEBUG: set $name $cmd ".join(" ", @val);
         # return FB_FORK_ConvertMOH $hash, @val;
      # }
   ...
   # } elsif ( lc $cmd eq 'convertringtone') {
      ...
      # {
         # FB_FORK_Log $hash, 5, "DEBUG: set $name $cmd ".join(" ", @val);
         # return FB_FORK_ConvertRingTone $hash, @val;
      # }
   } 
      ...
      {
         FB_FORK_Log $hash, 5, "DEBUG: set $name $cmd ".join(" ", @val);
         return FB_FORK_SetCustomerRingTone ($hash, @val);
      }
      ...
      if (int @val == 1 && $val[0] =~ /^(on|off)$/) {
         ...
         $state =~ s/off/0/;
         if ($forceShell) { # Shell
            FB_FORK_Shell_Exec( $hash, "ctlmgr_ctl w dect settings/enabled $state");
         } else { #webcm
            my @webCmdArray = ( ["dect:settings/enabled" => $state] );
            unless ($hash->{WEBCM}==1) {
               FB_FORK_Log $hash, 2, "'set ... dect' is not supported by the limited interfaces of your Fritz!Box firmware.";
               return "ERROR: 'set ... dect' is not supported by the limited interfaces of your Fritz!Box firmware.";
            }
            FB_FORK_Web_CmdPost ($hash, \@webCmdArray);
         }
      ...
      if ( int @val == 2 && $val[1] =~ /^(on|off)$/ ) {
         ...
         unless (defined $hash->{READINGS}{"diversity".$val[0]}) {
            FB_FORK_Log $hash, 2, "no diversity".$val[0]." to set.";
         ...
         $state =~ s/off/0/;
         if ($forceShell) { # Shell
            FB_FORK_Shell_Exec( $hash, "ctlmgr_ctl w telcfg settings/Diversity".( $val[0] - 1 )."/Active ".$state );
         }
            ...
            FB_FORK_TR064_Cmd ($hash, 0, \@tr064CmdArray);
         }
         else {
            FB_FORK_Log $hash, 2, "'set ... diversity' is not supported by the limited interfaces of your Fritz!Box firmware.";
      ...
      FB_FORK_Log $hash, 5, "DEBUG: set $name $cmd f�r Version: $FW1.$FW2 ";

      if ($FW1 <= 7 && $FW2 < 21) {
         FB_FORK_Log $hash, 2, "ERROR: FritzOS version must be greater than 7.20";
      ...
      if ( int @val == 2 && $val[0] =~ /^dect(\d+)$/ && $val[1] =~ /^(on|off)$/ ) {
	    ...
	    $end_mm   = "00";
         ...
         FB_FORK_Log $hash, 2, "ERROR: for dectringblock arguments";
         return "ERROR: for dectringblock arguments";
      }
	  
      if (ReadingsVal($name, $val[0], "nodect") eq "nodect") {
         FB_FORK_Log $hash, 2, "ERROR: dectringblock $val[0] not found";
	 ...
	 $queryStr .= "'endmm'        => '" . $end_mm . "'\n";
      ...
      FB_FORK_Log $hash, 4, "INFO: set $name $cmd " . join(" ", @webCmdArray);
	     
      FB_FORK_Log $hash, 5, "DEBUG: get $name $cmd " . $queryStr;
	  
      my $result = FB_FORK_Lua_Data( $hash, \@webCmdArray) ;

      my $tmp;
      if (defined $result->{Error} ) {      
         $tmp = "ERROR: " . $result->{Error}; 
         FB_FORK_Log $hash, 2, "ERROR: dectringblock " . $val[0] . " - " . $tmp;
      }
      ...
      if (int @val == 1 && $val[0] =~ /^(provider|other)$/) {
         ...
         FB_FORK_Log $hash, 4, "INFO: set $name $cmd f�r Version: $FW1.$FW2 ";
         ...
         FB_FORK_Log $hash, 3, "INFO: set $name $cmd " . join(" ", @val);
         ...
         if ( $val[0] eq "provider") {
           ...
           FB_FORK_Log $hash, 4, "INFO: data.lua: " . join(" ", @webCmdArray);
	  
           my $result = FB_FORK_Lua_Data( $hash, \@webCmdArray) ;

           if(defined $result->{Error}) {
              FB_FORK_Log $hash, 2, "ERROR: DNS IPv4 not set to " . $val[0] . " - " . $result->{Error};
              return "ERROR: setting DNS IPv4: " . $result->{Error};
           }

           FB_FORK_Log $hash, 4, "INFO: DNS IPv4 set to ".$val[0];
           ...
           $result = FB_FORK_Lua_Data( $hash, \@webCmdArray) ;
         ...
         FB_FORK_Log $hash, 2, "ERROR: for switchipv4dns arguments";
      ...
      if ( int @val == 2 && $val[1] =~ /^(on|off)$/ ) {
         ...
         if ( AttrVal( $name, "enableVPNShares", "0")) {
            ...
            unless (defined( $hash->{READINGS}{$val[0]})) {
               FB_FORK_Log $hash, 2, "ERROR: set $name $cmd " . join(" ", @val);
               return "ERROR: no $val[0] available."
            }

            FB_FORK_Log $hash, 4, "INFO: set $name $cmd " . join(" ", @val);
            push @cmdBuffer, "enablevpnshare " . join(" ", @val);
            return FB_FORK_Set_Cmd_Start $hash->{helper}{TimerCmd};

         } else {
            FB_FORK_Log $hash, 2, "ERROR: vpn readings not activated";
            return "ERROR: vpn readings not activated";
         }

      } else {
         FB_FORK_Log $hash, 2, "ERROR: for enableVPNshare arguments";
      ...
      if ( int @val == 1 && $val[0] =~ /^(on|off)$/ ) {
         ...
         FB_FORK_Log $hash, 2, "ERROR: for macFilter arguments";
      ...
      if (int @val == 2) {
         ...
         $val[0] = FB_FORK_Proof_Params($hash, $name, $cmd, "^(on|off)\$", @val);
         ...
         FB_FORK_Log $hash, 2, "ERROR: for locklandevice arguments";
      ...
      if(int @val == 2) {
         ...
         $val[0] = FB_FORK_Proof_Params($hash, $name, $cmd, "^filtprof(\\d+)\$", @val);
         ...
         FB_FORK_Log $hash, 2, "ERROR: for chgprofile arguments";
      ...
      {
         FB_FORK_Log $hash, 4, "INFO: set $name $cmd ".join(" ", @val);
         $resultStr = FB_FORK_SetMOH $hash, @val;
         if ($resultStr =~ /^[012]$/ )
         {
...
# set password
      ...
      if (int @val == 1) 
      {
         return FB_FORK_storePassword ( $hash, $val[0] );
      }
   }
#set Ring
      ...
      FB_FORK_SendMail_Shell $hash, @val;
      return undef;
   }
      ...
      if (int @val > 0) {
         ...
         return "'set ... startRadio' is not supported by the limited interfaces of your Fritz!Box firmware."
            unless $hash->{WEBCM}==1 || $forceShell;
         return FB_FORK_StartRadio_Web $hash, @val unless $forceShell;
         return FB_FORK_StartRadio_Shell $hash, @val;
      }
   } 
   elsif ( lc $cmd eq 'tam') {
      if ( int @val == 2 && defined( $hash->{READINGS}{"tam".$val[0]} ) && $val[1] =~ /^(on|off)$/ ) {
         ...
         $state =~ s/off/0/;
         if ($forceShell) { # Shell
            FB_FORK_Shell_Exec( $hash, "ctlmgr_ctl w tam settings/TAM".( $val[0] - 1 )."/Active ".$state );
         }
...
sub FB_FORK_Get($@)
{
   ...
   # get FB_FORK luaQuery inetstat:status/Today/BytesReceivedLow
   # get FB_FORK luaQuery telcfg:settings/AlarmClock/list(Name,Active,Time,Number,Weekdays)
      FB_FORK_Log $hash, 4, "INFO: get $name $cmd ".join(" ", @val);
      ...
      my $result = FB_FORK_Web_Query( $hash, $queryStr) ;
      
      my $tmp = FB_FORK_ERR_Result($hash, $result);
      ...
      my $result = FB_FORK_Web_Query( $hash, $val[0], "", "luaCall") ;
      #my $result = FB_FORK_Lua_Call( $hash, $val[0], $val[1]) ;
      ...
      FB_FORK_Log $hash, 4, "INFO: get $name $cmd [" . int(@val) . "] " . join(" ", @val);
      ...
      FB_FORK_Log $hash, 4, "INFO: get $name $cmd " . $queryStr;
      ...
      my $flag = 1;
      my $tmp = FB_FORK_ERR_Result($hash, $result, 1);
      ...
      my $erg = FB_FORK_Proof_Params($hash, $name, $cmd, "", @val);

      return $erg if($erg =~ /ERROR/);

      return FB_FORK_Lan_Device_Info( $hash, $erg, "info");
      ...
      if ( $val[0] eq "lanDevices") {
        $returnStr = FB_FORK_Lan_Devices_List($hash);
      } elsif ( $val[0] eq "vpnShares") {
        $returnStr = FB_FORK_VPN_Shares_List($hash);
      } elsif ( $val[0] eq "kidProfiles") {
        $returnStr = FB_FORK_Kid_Profiles_List($hash);
      } elsif ( $val[0] eq "userInfos") {
        $returnStr = FB_FORK_User_Info_List($hash);
      }
      ...
      unless $hash->{TELNET}==1;
      my $shCmd = join " ", @val;
      return FB_FORK_Shell_Exec( $hash, $shCmd );

   } elsif( lc $cmd eq "tr064command" && AttrVal( $name, "allowTR064Command", 0 ) ) {
# http://fritz.box:49000/tr64desc.xml
#get FB_FORK tr064command DeviceInfo:1 deviceinfo GetInfo
#get FB_FORK tr064command X_VoIP:1 x_voip X_AVM-DE_GetPhonePort NewIndex 1
#get FB_FORK tr064command X_VoIP:1 x_voip X_AVM-DE_DialNumber NewX_AVM-DE_PhoneNumber **612
#get FB_FORK tr064command X_VoIP:1 x_voip X_AVM-DE_DialHangup
#get FB_FORK tr064command WLANConfiguration:3 wlanconfig3 X_AVM-DE_GetWLANExtInfo
#get FB_FORK tr064command X_AVM-DE_OnTel:1 x_contact GetDECTHandsetList
#get FB_FORK tr064command X_AVM-DE_OnTel:1 x_contact GetDECTHandsetInfo NewDectID 1
#get FB_FORK tr064command X_AVM-DE_TAM:1 x_tam GetInfo NewIndex 0
#get FB_FORK tr064command X_AVM-DE_TAM:1 x_tam SetEnable NewIndex 0 NewEnable 0
#get FB_FORK tr064command InternetGatewayDevice:1 deviceinfo GetInfo
#get FB_FORK tr064command LANEthernetInterfaceConfig:1 lanethernetifcfg GetStatistics
      ...
      my @result = FB_FORK_TR064_Cmd( $hash, 1, \@tr064CmdArray );
      my $tmp = Dumper (@result);
      $returnStr .= $tmp;
      return $returnStr;

   } elsif( lc $cmd eq "tr064servicelist" ) {
      return FB_FORK_TR064_Get_ServiceList ($hash);
   }
...
sub FB_FORK_Proof_Params($@) {
   ...
   if ( int @val == 2 ) {
      unless ($val[1] =~ /$mysearch/ && ($val[0] =~ /^landevice(\d+)$/ || $val[0] =~ /^([0-9a-f]{2}([:-_]|$)){6}$/i) ) {
         $mysearch =~ s/\^|\$//g;
         FB_FORK_Log $hash, 2, "ERROR: no valid $cmd parameter: " . $val[0] . " or " . $mysearch . " given";
         return "ERROR: no valid $cmd parameter: " . $val[0] . " or " . $mysearch . " given";
      }
   } elsif ( int @val == 1 ) {
      if ($mysearch ne "" && $val[0] =~ /$mysearch/ ) {
        FB_FORK_Log $hash, 4, "INFO: $name $cmd " . join(" ", @val);
        return $val[0];
      } else {
         unless ( $val[0] =~ /^landevice(\d+)$/ || $val[0] =~ /^([0-9a-f]{2}([:-_]|$)){6}$/i ) {
            FB_FORK_Log $hash, 2, "ERROR: no valid $cmd parameter: " . $val[0] . " given";
            return "ERROR: no valid $cmd parameter: " . $val[0] . " given";
         }
      }

   } else {
      FB_FORK_Log $hash, 2, "ERROR: parameter missing";
   ...
   if ($val[0] =~ /^([0-9a-f]{2}([:-_]|$)){6}$/i) {
      ...
      if (exists($hash->{fhem}->{landevice}->{$mac}) eq "") {
         FB_FORK_Log $hash, 2, "ERROR: non existing landevice: $val[0]";
...
# First run is an API check
   unless ( $hash->{APICHECKED} ) {
      ...
      $runFn = "FB_FORK_API_Check_Run";
   }
# Run shell or web api, restrict interval
   else {
      $runFn = "FB_FORK_Readout_Run_Web";
      $runFn = "FB_FORK_Readout_Run_Shell"     if AttrVal( $name, "forceTelnetConnection",  0 ) == 1 || $hash->{REMOTE} == 0;
   }
   
   if( $interval != 0 ) {
      RemoveInternalTimer($hash->{helper}{TimerReadout});
      InternalTimer(gettimeofday()+$interval, "FB_FORK_Readout_Start", $hash->{helper}{TimerReadout}, 1);
   }

# Kill running process if "set update" is used
   if ( exists( $hash->{helper}{READOUT_RUNNING_PID} ) && $hash->{fhem}{LOCAL} == 1 ) {
      FB_FORK_Log $hash, 4, "INFO: Old readout process still running. Killing old process ".$hash->{helper}{READOUT_RUNNING_PID};
      BlockingKill( $hash->{helper}{READOUT_RUNNING_PID} ); 
   ...
   unless( exists $hash->{helper}{READOUT_RUNNING_PID} ) {
      $hash->{helper}{READOUT_RUNNING_PID} = BlockingCall($runFn, $name,
                                                       "FB_FORK_Readout_Done", 55,
                                                       "FB_FORK_Readout_Aborted", $hash);
      FB_FORK_Log $hash, 4, "INFO: Fork process $runFn";
   } 
   else {
      FB_FORK_Log $hash, 4, "INFO: Skip fork process $runFn";
   }

} # end FB_FORK_Readout_Start

# Checks which API is available on the Fritzbox
#######################################################################
sub FB_FORK_API_Check_Run($)
{
   my ($name) = @_;
   my $hash = $defs{$name};
   my $fritzShell = 0;
...
# if no FritzBoxIP is set, check if FHEM runs on a FritzBox under root user
   ...
   if ( $host =~ /undefined|local/ ) {
      ...
      if ( -X "/usr/bin/ctlmgr_ctl" ) {
         if ( $< != 0 ) {
            FB_FORK_Log $hash, 3, "INFO: Fhem is running on a Fritz!Box but not as 'root' user (currently " .
                                    ( getpwuid( $< ) )[ 0 ] . "). Cannot run in local mode.";
         }
         else {
            $fritzShell = 1;
            $host = "local"; # mark as local host
            FB_FORK_Log $hash, 3, "INFO: Fhem is running on a Fritz!Box as 'root' user.";
         }
      }
   }

# change host name if necessary
   FB_FORK_Readout_Add_Reading ($hash, \@roReadings, "->HOST", $host)      if $host ne $hash->{HOST};

# Determine local or remote mode
   if ($fritzShell) {
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "->REMOTE", 0;
      FB_FORK_Log $hash, 3, "INFO: FB_FORK modul runs in local mode.";
   }
   else {
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "->REMOTE", 1;
      FB_FORK_Log $hash, 3, "INFO: FB_FORK modul runs in remote mode.";
   }

# Check if perl modules for remote APIs exists
   if ($missingModulWeb) {
      FB_FORK_Log $hash, 3, "INFO: Cannot check for box model and APIs webcm, luaQuery and TR064 because perl modul $missingModulWeb is missing on this system.";
   }
# Check for remote APIs
   ...
   # Check if webcm exists
      ...
      if ($response->is_success) {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "->WEBCM", 1;
         FB_FORK_Log $hash, 4, "DEBUG: API webcm found (".$response->code.").";
      }
      else {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "->WEBCM", 0;
         FB_FORK_Log $hash, 3, "INFO: API webcm does not exist (".$response->status_line.")";
      }
   ...
   # Check if query.lua exists
      ...
      if ($response->is_success) {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "->LUAQUERY", 1;
         FB_FORK_Log $hash, 4, "DEBUG: API luaQuery found (".$response->code.").";
      }
      elsif ($response->code eq "500" || $response->code eq "403") {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "->LUAQUERY", 1;
         FB_FORK_Log $hash, 3, "INFO: API luaQuery found but responded with: ".$response->status_line;
      }
      else {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "->LUAQUERY", 0;
         FB_FORK_Log $hash, 3, "INFO: API luaQuery does not exist (".$response->status_line.")";
      }
   ...
   # Check if tr064 specification exists and determine TR064-Port
      ...
      if ($response->is_success) { #determine TR064-Port
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "->TR064", 1;
         FB_FORK_Log $hash, 4, "DEBUG: API TR-064 found.";
      #Determine TR064-Port
         my $tr064Port = FB_FORK_TR064_Init ( $hash, $host );
         if ($tr064Port) {
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, "->SECPORT", $tr064Port;
            FB_FORK_Log $hash, 4, "DEBUG: TR-064-SecurePort is $tr064Port.";
         }
         else {
            FB_FORK_Log $hash, 3, "INFO: TR-064-SecurePort does not exist";
         }
      }
      else {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "->TR064", 0;
         FB_FORK_Log $hash, 3, "INFO: API TR-064 does not exist: ".$response->status_line;
      }
   ...
   # Check if m3u can be created and the URL tested
      ...
      if (open my $fh, '>', $m3uFileLocal) {
         ...
         print $fh $ttsLink;
         close $fh;
         FB_FORK_Log $hash, 3, "INFO: Created m3u file '$m3uFileLocal'.";
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "->M3U_LOCAL", $m3uFileLocal;
      ...
      # if no URL and no local file defined, then try to build the correct URL 
         ...
         # Getting IP of FHEM host
            FB_FORK_Log $hash, 4, "DEBUG: Try to get my IP address.";
            ...
            $ip = $socket->sockhost if $socket; #A side-effect of making a socket connection is that our IP address is available from the 'sockhost' method
               FB_FORK_Log $hash, 3, "INFO: Could not determine my ip address"  unless $ip;
         # Get a web port
            my $port;
               FB_FORK_Log $hash, 4, "DEBUG: Try to get a FHEMWEB port.";
            foreach( keys %defs ) {
            ...
            FB_FORK_Log $hash, 3, "INFO: Could not find a FHEMWEB device."  unless $port;
      ...
      # Check if m3u can be accessed
         unless ( $m3uFileURL eq "unknown" ) {
            FB_FORK_Log $hash, 4, "DEBUG: Try to get '$m3uFileURL'";
            $response = $agent->get( $m3uFileURL );
            if ($response->is_error) {
               FB_FORK_Log $hash, 3, "ERROR: Failed to get '$m3uFileURL': ".$response->status_line;
               $m3uFileURL = "unknown"     ;
            }
         }
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "->M3U_URL", $m3uFileURL;
      } 
      else {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "->M3U_LOCAL", "undefined";
         FB_FORK_Log $hash, 2, "ERROR: Cannot create save file '$m3uFileLocal' because $!\n";
      }

   # Box model per jason
      FB_FORK_Log $hash, 5, "DEBUG: Read 'jason_boxinfo'";
      my $url = "http://".$host."/jason_boxinfo.xml";
      
      $response = $agent->get( $url );
      my $content  = $response->content;
      # FB_FORK_Log $hash, 5, "DEBUG: jason_boxinfo returned: $content";

      FB_FORK_Readout_Add_Reading ($hash, \@roReadings, "box_model", $1)   if ( $content =~ /<j:Name>(.*)<\/j:Name>/ );    
      
      FB_FORK_Readout_Add_Reading ($hash, \@roReadings, "box_oem", $1)    if $content =~ /<j:OEM>(.*)<\/j:OEM>/;
      FB_FORK_Readout_Add_Reading ($hash, \@roReadings, "box_fwVersion", $1)    if $content =~ /<j:Version>(.*)<\/j:Version>/;
    ...
    # Ansonsten Box-Model per system_status einlesen
      ...
      # Muss nochmal neu gesetzt werden, sonst gibt es einen Fehler (keine Ahnung warum)
         ...
         FB_FORK_Log $hash, 5, "DEBUG: Read 'system_status'";
         
         $response = $agent->get( $url );
         $content  = $response->content;
         # FB_FORK_Log $hash, 5, "DEBUG: system_status returned: $content";
         if ($response->is_success) {
            ...
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_model",  $result[0];
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_fwVersion", $result[7];
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_oem",    $result[9];
         }
         else {
            FB_FORK_Log $hash, 2, "ERROR: ".$response->status_line;
         };
      };
   }
...
# Check if telnet modul exists
   if ($missingModulTelnet) {
      FB_FORK_Log $hash, 3, "INFO: Cannot check for telnet access because perl modul $missingModulTelnet is missing on this system.\n";
   }
   else {
      my $timeout = AttrVal( $name, "telnetTimeOut", "10");
      my $telnet = new Net::Telnet ( Host=>$host, Port => 23, Timeout=>$timeout, Errmode=>'return', Prompt=>'/# $/');
      if (!$telnet) {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "->TELNET", 0;
         $telnet = undef;
         FB_FORK_Log $hash, 3, "ERROR: Could not open telnet connection to $host: $!";
      }   
      else {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "->TELNET", 1;
         $telnet->close;
         FB_FORK_Log $hash, 4, "INFO: Telnet connection availabel.";
      }
   }

   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "->APICHECKED", 1;

   push @roReadings, "readoutTime", sprintf( "%.2f", time()-$startTime);
   my $returnStr = join('|', @roReadings );

   FB_FORK_Log $hash, 4, "INFO: Captured " . @roReadings . " values";
   FB_FORK_Log $hash, 5, "DEBUG: Handover to main process (" . length ($returnStr) . "): " . $returnStr;
   
   return $name."|".encode_base64($returnStr,"");
   
} #end FB_FORK_API_Check_Run
   
# Starts the data capturing via Telnet and sets the new timer
#######################################################################
sub FB_FORK_Readout_Run_Shell($)
{
   my ($name) = @_;
   my $hash = $defs{$name};
   ...
   if ( int(time/3600) != $hash->{fhem}{lastHour} || $hash->{fhem}{LOCAL} != 0) {
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->lastHour", int(time/3600);
      $slowRun = 1;
      FB_FORK_Log $hash, 4, "INFO: Start update of slow changing device readings.";
   }
   else {
      FB_FORK_Log $hash, 4, "INFO: Start update of fast changing device readings.";
   }

   my $returnStr;
 
   $result = FB_FORK_Telnet_OpenCon( $hash );
   ...
   # Execute commands
      $resultArray = FB_FORK_Shell_Query( $hash, \@readoutCmdArray, \@roReadings);
   ...
   # Internetradioliste erzeugen
      ...
      for (0..$radioCount-1)
      {
         if ($resultArray->[$_] ne "")
         {
            ...
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->radio->".$_, $resultArray->[$_];
         }
      }
   ...
   # LanDevice-Liste erzeugen
      if ($lanDeviceCount > 0 )
      {
         ...
         for (0..$lanDeviceCount-1)
         {
            ...
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->landevice->$dIp", $dName;
            $landevice{$dIp}=$dName;
         }  
      }
...
# Dect Telefonnummern bestimmen
      ...
      for (1..$dectCount)
      {
         ...
         if ($intern)
         {
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->$intern->name", $resultArray->[ $offset + 1 ];
            $dectFonID{$ID}{User} = $_;
   ...
   # Handset and DECT user can be in different orders
      for (0..$handsetCount) {
         ...
         if ($dectUserID) {
            ...
            FB_FORK_Log $hash, 5, "DEBUG: dect".$dectUser."_manufacturer: " . $resultArray->[ $offset + 1 ];
            push @roReadings, "dect".$dectUser."_model|" . $resultArray->[ $offset + 2 ];
            FB_FORK_Log $hash, 5, "DEBUG: dect".$dectUser."_model: " . $resultArray->[ $offset + 2 ];
            push @roReadings, "dect".$dectUser."_fwVersion|" . $resultArray->[ $offset + 3 ];
            FB_FORK_Log $hash, 5, "DEBUG: dect".$dectUser."_fwVersion: " . $resultArray->[ $offset + 3 ];
...
# Prepare new command array
   ...
   # !!! Execute commands !!!
      FB_FORK_Shell_Query( $hash, \@readoutCmdArray, \@roReadings );
   }
...
# Alarm clock
   ...
   FB_FORK_Telnet_CloseCon ( $hash );

   FB_FORK_Log $hash, 4, "INFO: Captured " . @roReadings . " values";
   FB_FORK_Log $hash, 5, "DEBUG: Handover to main process (".length ($returnStr)."): ".$returnStr;
   return $name."|".encode_base64($returnStr,"");

} # End FB_FORK_Readout_Run_Shell

# http://fritz.box/cgi-bin/webcm?wlan:settings/guest_ap_enabled=1&sid=
      # FB_FORK_Log $hash, 3, "INFO: Web connection established with $sid";
      ...
      # my $response = $agent->post( $urlcgi,
         ...
         # ],
       # );
      # FB_FORK_Log $hash, 3, "DEBUG: ".$response->content;
   
# Starts the data capturing via query.lua and sets the new timer
#######################################################################
sub FB_FORK_Readout_Run_Web($)
{
   my ($name) = @_;
   my $hash = $defs{$name};
...
#Start update 
   FB_FORK_Log $hash, 4, "INFO: Prepare query string for luaQuery.";
   ...
   FB_FORK_Log $hash, 4, "ReadOut gestartet: $queryStr";
   $result = FB_FORK_Web_Query( $hash, $queryStr, "", "luaQuery") ;
   
   # Abbruch wenn Fehler beim Lesen der Fritzbox-Antwort
   if ( defined $result->{Error} ) {
      FB_FORK_Log $hash, 2, "ERROR: ".$result->{Error};
   ...
   if ( defined $result->{AuthorizationRequired} ) {
      FB_FORK_Log $hash, 2, "ERROR: AuthorizationRequired=".$result->{AuthorizationRequired};
   ...
   if ( ref $result->{wlanList} ne 'ARRAY' ) {
      FB_FORK_Log $hash, 4, "INFO: Recognized query answer of firmware >=6.69 and < 7";
      my $result2;
      my $newQueryPart; 
    ...
    # gets WLAN speed for fw>=6.69 and < 7
      ...
      foreach ( @{ $result->{wlanListNew} } ) {
         ...
         if (length($queryStr.$newQueryPart) < 4050) {
            ...
            $result2 = FB_FORK_Web_Query( $hash, $queryStr );
   ...
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->sid", $result->{sid};
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->sidTime", time();
...
# Dect-Geräteliste erstellen
   if ( $result->{handsetCount} =~ /[1-9]/ ) {
     ...
     foreach ( @{ $result->{dectUser} } ) {
        ...
        if ($intern) 
        {
           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "dect".$runNo,                     $_->{Name} ;
           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "dect".$runNo."_intern",           $intern ;
           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "dect".$runNo."_alarmRingTone",    $_->{AlarmRingTone0}, "ringtone" ;
           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "dect".$runNo."_intRingTone",      $_->{IntRingTone}, "ringtone" ;
           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "dect".$runNo."_radio",            $_->{RadioRingID}, "radio" ;
           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "dect".$runNo."_custRingTone",     $_->{G722RingTone} ;
           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "dect".$runNo."_custRingToneName", $_->{G722RingToneName} ;
           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "dect".$runNo."_imagePath",        $_->{ImagePath} ;

           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->$intern->id",   $id ;
           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->$intern->userId", $runNo;
  ...
  # Handset der internen Nummer zuordnen
     foreach ( @{ $result->{handset} } ) {
        ...
        if ($dectUser) {
           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "dect".$dectUser."_manufacturer", $_->{Manufacturer};
           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "dect".$dectUser."_model",        $_->{Model},         "model";
           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "dect".$dectUser."_fwVersion",    $_->{FWVersion};

           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->$intern->brand", $_->{Manufacturer};
           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->$intern->model", $_->{Model},       "model";
        }
     }
   }
# Analog Fons Name
   ...
   foreach ( @{ $result->{fonPort} } ) {
      if ( $_->{Name} )
      {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fon".$runNo,           $_->{Name};
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fon".$runNo."_out",    $_->{MSN};
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fon".$runNo."_intern", $runNo;
      }
      $runNo++;
   }
...
# Internetradioliste erzeugen
   ...
   foreach ( @{ $result->{radio} } ) {
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName,                 $_->{Name};
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->radio->".$runNo, $_->{Name};
      $runNo++;
      $rName = sprintf ("radio%02d",$runNo);
   }
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->radioCount", $runNo;

# SIP Lines
   my $boxModel = ReadingsNum($name, "box_model", "3490");
   FB_FORK_Log $hash, 4, "INFO: sip for box-model: " . $boxModel;
   ...
   if ($boxModel ne "3490" && AttrVal( $name, "enableSIP", "0")) {
      ...
      foreach ( @{ $result->{sip_info} } ) {
        FB_FORK_Log $hash, 4, "INFO: sip->info: " . $_->{_node} . ": " . $_->{activated};
        ...
        if ($_->{activated} == 1) {								# sip activated und registriert	
          ...
          if ($_->{connect} == 2) {								# sip activated und registriert	
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName, "active";
            delete $oldSIPDevice{$rName} if exists $oldSIPDevice{$rName};
            FB_FORK_Log $hash, 4, "INFO: $rName -> registration ok";
          }
          if ($_->{connect} == 0) {								# sip not activated	
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName, "inactive";
            delete $oldSIPDevice{$rName} if exists $oldSIPDevice{$rName};
            FB_FORK_Log $hash, 4, "INFO: $rName -> not active";
          }
          if ($_->{connect} == 1) {								# error condition for aktivated and unregistrated sips
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName, "not registered";
            delete $oldSIPDevice{$rName} if exists $oldSIPDevice{$rName};
            $sip_in_error++;
            FB_FORK_Log $hash, 2, "INFO: $rName -> not registered";
          }
        } else {
          FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName, "not in use";
          delete $oldSIPDevice{$rName} if exists $oldSIPDevice{$rName};
          FB_FORK_Log $hash, 4, "INFO: $rName -> not in use";
        }
   ...
   # Remove inactive or non existing sip-readings in two steps
      foreach ( keys %oldSIPDevice) {
         ...
         if ( $oldSIPDevice{$_} ne "inactive" ) {
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, $_, "inactive";
         } else {
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, $_, "";
         }
      }

      FB_FORK_Log $hash, 4, "INFO: end";
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "sip_error", $sip_in_error;
...
# VPN shares
   ...
   if ( AttrVal( $name, "enableVPNShares", "0")) {
     ...
     foreach ( @{ $result->{vpn_info} } ) {
       ...
       FB_FORK_Log $hash, 4, "INFO: vpn->info: $rName " . $_->{_node} . ": " . $_->{activated} . ": " . $_->{state};

       FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName, $_->{name};
       delete $oldVPNDevice{$rName} if exists $oldVPNDevice{$rName};

       FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName . "_access_type", "Corp VPN"    if $_->{access_type} == 1;
       FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName . "_access_type", "User VPN"    if $_->{access_type} == 2;
       FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName . "_access_type", "Lan2Lan VPN" if $_->{access_type} == 3;
       delete $oldVPNDevice{$rName . "_access_type"} if exists $oldVPNDevice{$rName . "_access_type"};

       FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName . "_remote_ip", $_->{remote_ip} eq "" ? "....":$_->{remote_ip};
       delete $oldVPNDevice{$rName . "_remote_ip"} if exists $oldVPNDevice{$rName . "_remote_ip"};

       FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName . "_activated", $_->{activated};
       delete $oldVPNDevice{$rName . "_activated"} if exists $oldVPNDevice{$rName . "_activated"};

       FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName . "_state", $_->{state} eq "" ? "none":$_->{state};
       delete $oldVPNDevice{$rName . "_state"} if exists $oldVPNDevice{$rName . "_state"};

       if ($_->{connected_since} == 0) {
          FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName . "_connected_since", $_->{connected_since};
       } else {
          ...
          $Min = substr("0".$Min,-2);
          $Sek = substr("0".$Sek,-2);
          FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName . "_connected_since", $_->{connected_since} . " sec = " . $Tag . "T $Std:$Min:$Sek";
       }
...
# Create WLAN-List
   ...
   if ( ref $result->{wlanList} eq 'ARRAY' ) {
      foreach ( @{ $result->{wlanList} } ) {
         ...
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->wlanDevice->".$mac."->speed", $_->{speed};
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->wlanDevice->".$mac."->speed_rx", $wlanList{$mac}{speed_rx};
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->wlanDevice->".$mac."->rssi", $_->{rssi};
      }
   }
...
# Create LanDevice list and delete inactive devices
   ...
   if ( ref $result->{lanDevice} eq 'ARRAY' ) {
      foreach ( @{ $result->{lanDevice} } ) {
         ...
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->landevice->$dIp", $dName;
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->landevice->$UID", $dName;
         $landevice{$dIp}=$dName;
         $landevice{$UID}=$dName;
      ...
      # Create a reading if a landevice is connected
         if ( $_->{active} || $allowPassiv) {
            ...
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->landevice->$mac", $UID;
            $landevice{$mac}=$UID;
            ...
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName, $dName;
            # $wlanCount++      if $_->{wlan} ;
   ...
   FB_FORK_Readout_Add_Reading ($hash, \@roReadings, "box_wlanCount", $wlanCount) unless((grep { /^(box_wlanCount)$/ } @reading_list));
   FB_FORK_Readout_Add_Reading ($hash, \@roReadings, "box_guestWlanCount", $gWlanCount) unless((grep { /^(box_guestwlanCount)$/ } @reading_list));
...
# WLANs
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_wlan_2.4GHz", $result->{box_wlan_24GHz}, "onoff" unless((grep { /^(box_wlan_24GHz)$/ } @reading_list));
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_wlan_5GHz", $result->{box_wlan_5GHz}, "onoff" unless((grep { /^(box_wlan_5GHz)$/ } @reading_list));
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_guestWlan", $result->{box_guestWlan}, "onoff" unless((grep { /^(box_guestWlan)$/ } @reading_list));
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_guestWlanRemain", $result->{box_guestWlanRemain} unless((grep { /^(box_guestWlanRemain)$/ } @reading_list));
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_macFilter_active", $result->{box_macFilter_active}, "onoff" unless((grep { /^(box_macFilter_active)$/ } @reading_list));
# Dect
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_dect", $result->{box_dect}, "onoff" unless((grep { /^(box_dect)$/ } @reading_list));
# Music on Hold
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_moh", $result->{box_moh}, "mohtype" unless((grep { /^(box_moh)$/ } @reading_list));
# Power Rate
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_powerRate", $result->{box_powerRate} unless((grep { /^(box_powerRate)$/ } @reading_list));
# Box Features
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->is_double_wlan", $result->{is_double_wlan}, "01";
# Box model, firmware and uptimes
   ...
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_uptime", $Sek . " sec = " . $Tag . "T $Std:" . substr("0".$result->{box_uptimeMinutes},-2) . ":00" unless((grep { /^(box_uptime)$/ } @reading_list));
   ...
   if ( AttrVal( $name, "allowTR064Command", 0 )) {
      ...
      FB_FORK_Log $hash, 5, "DEBUG: Curl-> " . $strCurl;

      if($strCurl =~ m/<NewConnectionStatus>(.*?)<\/NewConnectionStatus>/i) {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_connect", $1 unless((grep { /^(box_connect)$/ } @reading_list));
      }
      if($strCurl =~ m/<NewLastConnectionError>(.*?)<\/NewLastConnectionError>/i) {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_last_connect_err", $1 unless((grep { /^(box_connect_err)$/ } @reading_list));
      }
      ...
      if($strCurl =~ m/<NewUptime>(.*?)<\/NewUptime>/i) {
         ...
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_uptimeConnect", $1 . " sec = " . $Tag . "T $Std:$Min:$Sek" unless((grep { /^(box_uptimeConnect)$/ } @reading_list));
      }
      ...
      if($strCurl =~ m/<NewExternalIPAddress>(.*?)<\/NewExternalIPAddress>/i) {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_ipExtern", $1 unless(grep { /^(box_ipExtern)$/ } @reading_list);
      }
      ...
      if($strCurl =~ m/<NewExternalIPv6Address>(.*?)<\/NewExternalIPv6Address>/i) {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_ipv6Extern", $1 unless(grep { /^(box_ipv6Extern)$/ } @reading_list);
      }
      ...
      if($strCurl =~ m/<NewIPv6Prefix>(.*?)<\/NewIPv6Prefix>/i) {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_ipv6Prefix", $1 unless(grep { /^(box_ipv6Prefix)$/ } @reading_list);
      }

      if ($result->{box_fwVersion}) {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_fwVersion", $result->{box_fwVersion} unless(grep { /^(box_fwVersion)$/ } @reading_list);
      } else { # Ab Version 6.90
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_fwVersion", $result->{box_fwVersion_neu} unless(grep { /^(box_fwVersion_neu)$/ } @reading_list);
      }

      my @tr064CmdArray = (["WANDSLInterfaceConfig:1", "wandslifconfig1", "GetInfo"]);
      my @tr064Result = FB_FORK_TR064_Cmd( $hash, 0, \@tr064CmdArray );
      if ($tr064Result[0]->{Error}) {
        $strCurl = Dumper (@tr064Result);
        FB_FORK_Log $hash, 2, "ERROR: Curl-> " . $strCurl;
      } else {
        $strCurl = Dumper (@tr064Result);
        FB_FORK_Log $hash, 5, "DEBUG: Curl-> " . $strCurl;
        $strCurl =~ /'NewDownstreamCurrRate' => '(.*)'/;
        FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_vdsl_downStreamRate", $1/1000 unless(grep { /^(box_vdsl_downStreamRate)$/ } @reading_list);
        $strCurl =~ /'NewUpstreamCurrRate' => '(.*)'/;
        FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_vdsl_upStreamRate", $1/1000 unless(grep { /^(box_vdsl_upStreamRate)$/ } @reading_list);
      }
   }

   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_fwUpdate",    $result->{box_fwUpdate} unless(grep { /^(box_fwUpdate)$/ } @reading_list);
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_tr064",       $result->{box_tr064},       "onoff" unless(grep { /^(box_tr064)$/ } @reading_list);
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_tr069",       $result->{box_tr069},       "onoff" unless(grep { /^(box_tr069)$/ } @reading_list);
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_stdDialPort", $result->{box_stdDialPort}, "dialport" unless(grep { /^(box_stdDialPort)$/ } @reading_list);
   # FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_ipExtern",    $result->{box_ipExtern};
   # FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_connect",     $result->{box_connect};
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_cpuTemp",     $result->{box_cpuTemp} unless(grep { /^(box_cpuTemp)$/ } @reading_list);
   if ( defined ($result->{dslStatGlobalOut}) && looks_like_number($result->{dslStatGlobalOut}) ) {
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_dsl_upStream", sprintf ("%.3f", $result->{dslStatGlobalOut}/1000000) unless (grep { /^(box_dsl_upStream)$/ } @reading_list);
   }
   if ( defined ($result->{dslStatGlobalIn}) && looks_like_number($result->{dslStatGlobalIn}) ) {   
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_dsl_downStream", sprintf ("%.3f", $result->{dslStatGlobalIn}/1000000) unless (grep { /^(box_dsl_downStream)$/ } @reading_list) ;
   }

# GSM
#FB_FORK_Readout_Add_Reading $hash, \@roReadings, "gsm_modem", $result->{GSM_ModemPresent};
   if (defined $result->{GSM_NetworkState} && $result->{GSM_NetworkState} ne "0") {
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "gsm_rssi", $result->{GSM_RSSI};
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "gsm_state", $result->{GSM_NetworkState}, "gsmnetstate";
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "gsm_technology", $result->{GSM_AcT}, "gsmact";
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "gsm_internet", $result->{UMTS_enabled};
   } else {
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "gsm_rssi", "";
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "gsm_state", "";
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "gsm_technology", "";
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "gsm_internet", "";
   }
...
# Alarm clock
   ...
   foreach ( @{ $result->{alarmClock} } ) {
      ...
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "alarm".$runNo, $_->{Name};
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "alarm".$runNo."_state", $_->{Active}, "onoff";
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "alarm".$runNo."_time",  $_->{Time}, "altime";
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "alarm".$runNo."_target", $_->{Number}, "alnumber";
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "alarm".$runNo."_wdays", $_->{Weekdays}, "aldays";
      $runNo++;
   }
...
#Get TAM readings
   ...
   foreach ( @{ $result->{tam} } ) {
      ...
      {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName,           $_->{Name};
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName."_state",  $_->{Active}, "onoff";
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName."_newMsg", $_->{NumNewMessages};
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName."_oldMsg", $_->{NumOldMessages};
      }
# L�schen ausgeblendeter TAMs
      elsif (defined $hash->{READINGS}{$rName} )
      {
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName,"";
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName."_state", "";
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName."_newMsg","";
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName."_oldMsg","";
      }
      $runNo++;
   }
   ...
   if ( ref $result->{userProfil} eq 'ARRAY' ) {
      ...
      # do not show data for unlimited, blocked or default access rights
         if ($_->{filter_profile_UID} !~ /^filtprof[134]$/ || defined $hash->{READINGS}{$rName} ) {
            ...
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName,                   $_->{name},            "deviceip";
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName."_thisMonthTime",  $_->{this_month_time}, "secondsintime";
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName."_todayTime",      $_->{today_time},      "secondsintime";
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName."_todaySeconds",   $_->{today_time};
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName."_type",           $_->{type},            "usertype";
         }
         $runNo++;
...
# user ticket (extension of online time)
   if ( ref $result->{userTicket} eq 'ARRAY' ) {
      ...
      foreach ( @{ $result->{userTicket} } ) {
         ...
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName, $_->{id};
         $runNo++;
   ...
   foreach ( @{ $result->{diversity} } ) {
     FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName,          $_->{MSN};
     FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName."_state", $_->{Active}, "onoff" ;
     FB_FORK_Readout_Add_Reading $hash, \@roReadings, $rName."_dest",  $_->{Destination};
      $runNo++;
...
# attr global showInternalValues 0
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, ".box_TodayBytesReceivedHigh", $result->{TodayBytesReceivedHigh};
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, ".box_TodayBytesReceivedLow", $result->{TodayBytesReceivedLow};
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, ".box_TodayBytesSentHigh", $result->{TodayBytesSentHigh};
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, ".box_TodayBytesSentLow", $result->{TodayBytesSentLow};
...
} # End FB_FORK_Readout_Run_Web

#######################################################################
sub FB_FORK_Readout_Done($)
{
   my ($string) = @_;
   ...
   FB_FORK_Log $hash, 5, "DEBUG: Back at main process";

# delete the marker for RUNNING_PID process
   delete($hash->{helper}{READOUT_RUNNING_PID});

   $string2 = decode_base64($string2);
   FB_FORK_Readout_Process ($hash, $string2);
...
sub FB_FORK_Readout_Process($$)
{
 ...
 # Fatal Error: no hash parameter handed over
   ...
   FB_FORK_Log $hash, 4, "INFO: Processing " . keys(%values) . " readouts.";
   ...
   if ( defined $values{Error} ) {
      ...
      if (defined $values{"fhem->sidTime"}) {
         ...
         FB_FORK_Log $hash, 5, "DEBUG: Reset SID";
      }
   }
   else {
   # Statistics
      ...
      #hash values
         ...
         if ($rName !~ /->|box_fwUpdate|box_oem|readoutTime/) {
            if ($rValue ne "") {
               ...
               FB_FORK_Log $hash, 5, "DEBUG: SET $rName = '$rValue'";
            }
            elsif ( exists $hash->{READINGS}{$rName} ) {  
               delete $hash->{READINGS}{$rName};
               FB_FORK_Log $hash, 5, "DEBUG: Delete reading $rName.";
            }
            else  {
               FB_FORK_Log $hash, 5, "DEBUG: Ignore reading $rName.";
            }
         }
      }
   ...
   # Create state with wlan states
      if ( defined $values{"box_wlan_2.4GHz"} ) {
         ...
         FB_FORK_Log $hash, 5, "DEBUG: SET state = '$newState'";
      }

   # adapt TR064-Mode
      if ( defined $values{box_tr064} ) {
         if ( $values{box_tr064} eq "off" && defined $hash->{SECPORT} ) {
               FB_FORK_Log $hash, 3, "INFO: TR-064 is switched off";
            delete $hash->{SECPORT};
         }
         elsif ( $values{box_tr064} eq "on" && not defined $hash->{SECPORT} ) {
               FB_FORK_Log $hash, 3, "INFO: TR-064 is switched on";
            my $tr064Port = FB_FORK_TR064_Init ($hash, $hash->{HOST});
            $hash->{SECPORT} = $tr064Port    if $tr064Port;
         }
      }
...
sub FB_FORK_Readout_Aborted($)
{
  ...
  FB_FORK_Log $hash, 1, "INFO: " . $msg;
}

#######################################################################
sub FB_FORK_Readout_Format($$$)
{
...
sub FB_FORK_Readout_Add_Reading ($$$$@)
{
   ...
   $rValue = FB_FORK_Readout_Format ($hash, $rFormat, $rValue);
   push @{$roReadings}, $rName . "|" . $rValue;

   if ($rName eq ",") {
     FB_FORK_Log $hash, 1, "ERROR: $rName: $rValue from caller -> $sub.$xline";
   }

   FB_FORK_Log $hash, 5, "DEBUG: $rName: $rValue";
}
...
sub FB_FORK_Set_Cmd_Start($)
{
 ...
 # kill old process if timeout + 10s is reached
   if ( exists( $hash->{helper}{CMD_RUNNING_PID}) && time()> $cmdBufferTimeout + 10 ) {
      FB_FORK_Log $hash, 1, "INFO: Old command still running. Killing old command: ".$cmdBuffer[0];
      shift @cmdBuffer;
      BlockingKill( $hash->{helper}{CMD_RUNNING_PID} ); 
 ...
 # (re)start timer if command buffer is still filled
   if (int @cmdBuffer >1) {
      RemoveInternalTimer($hash->{helper}{TimerCmd});
      InternalTimer(gettimeofday()+1, "FB_FORK_Set_Cmd_Start", $hash->{helper}{TimerCmd}, 1);
   }
...
# Preparing SET Call
   if ($val[0] eq "call") {
      ...
      $cmdFunction = "FB_FORK_Call_Run_Web";
      $cmdFunction = "FB_FORK_Call_Run_Shell" if $forceShell;
   }
# Preparing SET guestWLAN
      ...
      $cmdBufferTimeout = time() + $timeout;
      $handover = $name . "|" . join( "|", @val );
      $cmdFunction = "FB_FORK_GuestWlan_Run_Web";
      $cmdFunction = "FB_FORK_GuestWlan_Run_Shell" if $forceShell;
   }
# Preparing SET RING
   elsif ($val[0] eq "ring") {
      shift @val;
      ...
      $cmdBufferTimeout = time() + $timeout;
      $handover = $name . "|" . join( "|", @val );
      $cmdFunction = "FB_FORK_Ring_Run_Web";
      $cmdFunction = "FB_FORK_Ring_Run_Shell" if $forceShell;
   }
# Preparing SET WLAN
   elsif ($val[0] eq "wlan") {
      $timeout = 10;
      $cmdBufferTimeout = time() + $timeout;
      $handover = $name . "|" . join( "|", @val );
      $cmdFunction = "FB_FORK_Wlan_Run_Web";
      $cmdFunction = "FB_FORK_Wlan_Run_Shell" if $forceShell;
   }
...
# Preparing SET macFilter
      ...
      $cmdBufferTimeout = time() + $timeout;
      $handover = $name . "|" . join( "|", @val );
      $cmdFunction = "FB_FORK_Run_macFilter";
   }
# Preparing SET chgProfile
      ...
      $cmdBufferTimeout = time() + $timeout;
      $handover = $name . "|" . join( "|", @val );
      $cmdFunction = "FB_FORK_Run_chgProfile";
   }
# Preparing SET lockLandevice
      ...
      $cmdBufferTimeout = time() + $timeout;
      $handover = $name . "|" . join( "|", @val );
      $cmdFunction = "FB_FORK_Run_lockLandevice";
   }
# Preparing SET enableVPNshare
      ...
      $cmdBufferTimeout = time() + $timeout;
      $handover = $name . "|" . join( "|", @val );
      $cmdFunction = "FB_FORK_Run_enableVPNshare";
   }
...
# Starting new command
   FB_FORK_Log $hash, 4, "INFO: Fork process $cmdFunction";
   $hash->{helper}{CMD_RUNNING_PID} = BlockingCall($cmdFunction, $handover,
                                       "FB_FORK_Set_Cmd_Done", $timeout,
                                       "FB_FORK_Set_Cmd_Aborted", $hash);
   return undef;
} # end FB_FORK_Set_Cmd_Start

#######################################################################
sub FB_FORK_Set_Cmd_Done($)
{
   my ($string) = @_;
  unless (defined $string)
   {
   ...
   FB_FORK_Log $hash, 4, "INFO: Back at main process";
   ...
   if ( $success !~ /1|2/ )
   {
      FB_FORK_Log $hash, 1, "ERROR: " . $result;
      FB_FORK_Readout_Process ( $hash, "Error|" . $result );
   }
   elsif ( $success == 1 )
   {
      FB_FORK_Log $hash, 4, "INFO: " . $result;
   }
   elsif  ($success == 2 )
   {
      $result = decode_base64($result);
      FB_FORK_Readout_Process ( $hash, $result );
   }
}

#######################################################################
sub FB_FORK_Set_Cmd_Aborted($)
{
  my ($hash) = @_;
  my $lastCmd = shift (@cmdBuffer);
  delete($hash->{helper}{CMD_RUNNING_PID});
  FB_FORK_Log $hash, 1, "INFO: Timeout reached for: $lastCmd";
}
...
sub FB_FORK_Run_macFilter($)
{
   ...
   $result = FB_FORK_Web_Query( $hash, $queryStr) ;

   if ( defined $result->{Error} ) {
      FB_FORK_Log $hash, 2, "ERROR: macFilter -> " . $result->{Error};
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "macFilterStat", "macFilter->ERROR: " . $result->{Error};
   } elsif ( defined $result->{AuthorizationRequired} ) {
      FB_FORK_Log $hash, 2, "ERROR: AuthorizationRequired -> " . $result->{AuthorizationRequired};
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "macFilterStat", "macFilter->ERROR: AuthorizationRequired";
   } elsif ( ! defined ($result->{box_macFilter_active}) ) {
#         unless( defined ($result->{box_macFilter_active}) ) {
      FB_FORK_Log $hash, 2, "ERROR: MAC Filter not available";
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "macFilterStat", "macFilter->ERROR: MAC Filter not available";
   } elsif ( $switch == $result->{box_macFilter_active} ) {
      FB_FORK_Log $hash, 4, "INFO: no macFilter change necessary";
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "macFilterStat", "macFilter->INFO: change necessary";
   } else {
      ...
      FB_FORK_Log $hash, 5, "DEBUG: set $name $cmd " . join(" ", @webCmdArray);
      ...
      if(defined $result->{Error}) {
         FB_FORK_Log $hash, 2, "ERROR: setting MAC Filter: " . $result->{Error};
         ...
         if ( defined $result->{Error} ) {
            ...
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, "box_macFilter_active", $val[0];

            FB_FORK_Log $hash, 4, "INFO: macFilter set to " . $val[0];
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, "macFilterStat", "macFilter->set to " . $val[0];
         }
      }
   }

   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "readoutTime", sprintf( "%.2f", time()-$startTime);
   
   $returnStr = join('|', @roReadings );
   FB_FORK_Log $hash, 5, "DEBUG: Handover to main process: " . $returnStr;
...
sub FB_FORK_Run_chgProfile($)
{
   ...
   $result = FB_FORK_Lua_Data( $hash, \@webCmdArrayP) ;

   if(defined $result->{Error}) {
      FB_FORK_Log $hash, 2, "ERROR: changing Kid Profile: " . $result->{Error};
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "chgProfileStat", $val[1] . "->ERROR: " . $result->{Error};
      ...
      eval {
         foreach my $key (keys %$views) {
            FB_FORK_Log $hash, 5, "DEBUG: Kid Profiles: ".$key;
      ...
      if ($ProfileOK eq "false") {
         FB_FORK_Log $hash, 2, "ERROR: " . $val[1] . " not available/defined.";
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "chgProfileStat", $val[1] . "->ERROR: not available/defined";
      } else {

         FB_FORK_Log $hash, 4, "INFO: Profile $val[1] available.";

         my $lanDevice_Info = FB_FORK_Lan_Device_Info( $hash, $val[0], "chgProf");

         return $lanDevice_Info if $lanDevice_Info =~ /ERROR/;

         FB_FORK_Log $hash, 5, "DEBUG: \n" . Dumper $lanDevice_Info;
         ...
         if($lanDevice_Info->{data}->{vars}->{dev}->{UID} eq $val[0]) {
            ...
            FB_FORK_Log $hash, 3, "INFO: set $name $cmd " . join(" ", @webCmdArray);
            ...
            if( substr($tmp, 0, 6) eq "ERROR:") {
               FB_FORK_Log $hash, 2, "ERROR: result $name $cmd " . $tmp;
               FB_FORK_Readout_Add_Reading $hash, \@roReadings, "chgProfileStat", $val[0] . "->ERROR: changing profile";
            } else {
               FB_FORK_Log $hash, 4, "INFO: result $name $cmd " . $tmp;
               FB_FORK_Readout_Add_Reading $hash, \@roReadings, "chgProfileStat", $val[0] . "->INFO: profile ". $val[1];
            }
           
         } else {
            FB_FORK_Log $hash, 2, "ERROR: " . $val[0] . " not available/defined.";
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, "chgProfileStat", $val[0] . "->ERROR: not available/defined";
         }
      }
   }
...
sub FB_FORK_Run_enableVPNshare($)
{
   ...
   # Abbruch wenn Fehler beim Lesen der Fritzbox-Antwort
   if ( defined $result->{Error} ) {
     FB_FORK_Log $hash, 2, "ERROR: evaluating $val[0] -> general error: " . $result->{Error};
     FB_FORK_Readout_Add_Reading $hash, \@roReadings, "enableVPNshareStat", $val[0] . "->ERROR: " . $result->{Error};
   } elsif ( defined $result->{AuthorizationRequired} ) {
     FB_FORK_Log $hash, 2, "ERROR: evaluating vpn$val[0] -> AuthorizationRequired";
     FB_FORK_Readout_Add_Reading $hash, \@roReadings, "enableVPNshareStat", $val[0] . "->ERROR: AuthorizationRequired";
   } else {
     ...
     if ($vpnok == 0){
       FB_FORK_Log $hash, 2, "ERROR: vo valid " . $val[0] . " found";
       FB_FORK_Readout_Add_Reading $hash, \@roReadings, "enableVPNshareStat", $val[0] . "->ERROR: not found";
     } else {
       FB_FORK_Log $hash, 4, "INFO: set $name $cmd " . join(" ", @val);
       ...
       FB_FORK_Log $hash, 5, "DEBUG: data.lua: \n" . join(" ", @webCmdArray);
	  
       $result = FB_FORK_Lua_Data( $hash, \@webCmdArray) ;

       if(defined $result->{Error}) {
         FB_FORK_Log $hash, 2, "ERROR: enable $val[0] share: " . $result->{Error};
         FB_FORK_Readout_Add_Reading $hash, \@roReadings, "enableVPNshareStat", $val[0] . "->ERROR: " . $result->{Error};
       } else {

         $queryStr = "&vpn_info=vpn:settings/connection$vpnShare/activated";
         my $vpnState = FB_FORK_Web_Query( $hash, $queryStr) ;

         FB_FORK_Log $hash, 5, "DEBUG: $vpnState->{vpn_info} \n" . Dumper $vpnState;

         if(defined $vpnState->{Error}) {
            FB_FORK_Log $hash, 2, "ERROR: enable $val[0] share: " . $vpnState->{Error};
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, "enableVPNshareStat", $val[0] . "->ERROR: " . $vpnState->{Error};
         } elsif ($vpnState->{vpn_info} != $state) {
            FB_FORK_Log $hash, 2, "ERROR: VPNshare " . $val[0] . " not set to " . $val[1] . " <> " . $vpnState->{vpn_info};
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, "enableVPNshareStat", $val[0] . "->ERROR: " . $vpnState->{vpn_info};
         } else {
            FB_FORK_Log $hash, 4, "INFO: VPNshare " . $val[0] . " set to " . $val[1];
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, "enableVPNshareStat", $val[0] . "->" . $val[1];
            FB_FORK_Readout_Add_Reading $hash, \@roReadings, $val[0] . "_activated", $vpnState->{vpn_info};
         }
       }
     }
   }
...
sub FB_FORK_Run_lockLandevice($) 
{
   ...
   my $lock_res = FB_FORK_Lan_Device_Info( $hash, $val[0], "lockLandevice");
   ...
   unless (($lock_res =~ /blocked/ && $val[1] eq "on") || ($lock_res =~ /unlimited/ && $val[1] eq "off")) {
     ...
     if ( defined $result->{Error} ) {
       FB_FORK_Log $hash, 2, "ERROR: lockLandevice status: " . $result->{Error};
       FB_FORK_Readout_Add_Reading $hash, \@roReadings, "lockLandevStat", $val[0] . "->ERROR: " . $result->{Error};
     } else {

       $lock_res = FB_FORK_Lan_Device_Info( $hash, $val[0], "lockLandevice");

       if ($lock_res =~ /ERROR/) {
          FB_FORK_Log $hash, 2, "ERROR: setting locklandevice: " . substr($lock_res, 7);
          FB_FORK_Readout_Add_Reading $hash, \@roReadings, "lockLandevStat", $val[0] . "->ERROR:" . substr($lock_res, 7);
       } else {

         unless (($lock_res =~ /blocked/ && $val[1] eq "on") || ($lock_res =~ /unlimited/ && $val[1] eq "off")) {
           FB_FORK_Log $hash, 2, "ERROR: setting locklandevice: " . $val[0];
           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "lockLandevStat", $val[0] . "->ERROR: setting locklandevice " . $val[1];
         } else {
           FB_FORK_Log $hash, 4, "INFO: " . $lock_res . " -> $name $cmd $val[1]";
           FB_FORK_Readout_Add_Reading $hash, \@roReadings, "lockLandevStat", $val[0] . "->" . $val[1];
         }
       }
     }
   } else {
...
sub FB_FORK_Call_Run_Shell($) 
{
 ...
 # Check if 2nd parameter is the duration
   ...
   if (int @val)
   {
      ...
      {
         $duration = $val[0];
         FB_FORK_Log $hash, 5, "DEBUG: Extracted call duration of $duration s.";
         shift @val;
      }
   }
...
# Create tts link to say as moh
   if ( $field{say} ) 
   {
      ...
      {
         FB_FORK_Log $hash, 2, "ERROR: Cannot do Text2Speech because box has no music on hold";
      }
      else
      {
         chop $field{say};
         ...
         $ttsLink =~ s/\[TEXT\]/$ttsText/;
         FB_FORK_Log $hash, 5, "DEBUG: Created Text2Speech internet link: $ttsLink";
      }
   }
   ...
   if ($field{play})
   {
      unless ($hash->{READINGS}{box_moh})
      {
         FB_FORK_Log $hash, 2, "ERROR: Cannot play mp3 because box has no music on hold";
      }
      elsif ($ttsLink)
      {
         FB_FORK_Log $hash, 3, "INFO: Ignore 'play:' because Text2Speech already defined.";
      }
      else
      {
         chop $field{play};
         $ttsLink = $field{play};
         FB_FORK_Log $hash, 5, "DEBUG: Extracted MP3 ring tone: $ttsLink";
      }
   }
...
# Execute 1st command array
      $result = FB_FORK_Shell_Exec ( $hash, \@cmdArray );
   ...
   #Execute 3rd command array
      FB_FORK_Shell_Exec( $hash, \@cmdArray );
...
# switch to (dial port 1-3) to avoid ringing of internal phone
   ...
   FB_FORK_Log $hash, 4, "INFO: Call $extNo for $duration seconds";
...
} # End FB_FORK_Call_Run_Shell

#######################################################################
sub FB_FORK_Call_Run_Web($) 
{
   my ($string) = @_;
   my ($name, @val) = split "\\|", $string;
   my $hash = $defs{$name};
...
# Create tts link to say as moh
   ...
   # Execute 1st command array
         $result = FB_FORK_Shell_Exec ( $hash, \@shellCmdArray );
         return "$name|0|Could not access '$ttsLink'"
            unless $result->[4] eq "1";
         return "$name|0|Could locate '/var/flash/fx_moh'"
            unless $result->[5] eq "1";
      ...
      #Execute 3rd command array
         FB_FORK_Shell_Exec( $hash, \@shellCmdArray );
      ...
      FB_FORK_Log $hash, 3, "INFO: Your Fritz!OS version has limited interfaces. Parameter 'play:' and 'say:' ignored.";
   }
...
# Preparing 4th command array to switch to (dial port 1-3) to avoid ringing of internal phone
   ...
   if ($ringWithIntern =~ /^([1-3])$/ && $hash->{WEBCM} == 1 ) {
      ...
      $result = FB_FORK_Web_CmdPost( $hash, \@webCmdArray );
   }
...
#Preparing 5th command array to ring
   ...
   if ( $hash->{WEBCM} == 1 ) { # ring with webcm
      ...
      $result = FB_FORK_TR064_Cmd( $hash, 0, \@tr064CmdArray );
   }
   else {
      FB_FORK_Log $hash, 3, "INFO: Your Fritz!OS version has limited interfaces. You cannot use call.";
   }
...
#Preparing 5th and 6th command array to stop ringing and reset dial port
   if ( $hash->{WEBCM} == 1 ) { # hangup with webcm
      ...
      $result = FB_FORK_TR064_Cmd( $hash, 0, \@tr064CmdArray )   if $hash->{SECPORT};
   }
...
} # End FB_FORK_Call_Run_Web

#######################################################################
sub FB_FORK_GuestWlan_Run_Shell($)
{
   my ($string) = @_;
   my ($name, @val) = split "\\|", $string;
   my $hash = $defs{$name};
   ...
   $result = FB_FORK_Shell_Exec $hash, "[ -n `ctlmgr_ctl r wlan settings/guest_pskvalue` ] && echo 1 || echo 0";
...
} # end FB_FORK_GuestWlan_Run_Shell

#######################################################################
sub FB_FORK_GuestWlan_Run_Web($)
{
   my ($string) = @_;
   my ($name, @val) = split "\\|", $string;
   my $hash = $defs{$name};
...
# Set guestWLAN, if necessary set also WLAN
   if ( $hash->{WEBCM}==1 ) { #webcm
      ...
      # FB_FORK_Web_CmdPost ($hash, \@webCmdArray, '/wlan/wlan_settings.lua');
      ...
      FB_FORK_Log $hash, 2, "ERROR: No API available to switch WLAN.";
   }

   # push @webCmdArray, "autoupdate" => "on";
   # push @webCmdArray, "activate_guest_access" => $val[0];
   # FB_FORK_Web_CmdPost ($hash, \@webCmdArray, '/wlan/guest_access.lua');
...
} # end FB_FORK_GuestWlan_Run_Web

#######################################################################
sub FB_FORK_Wlan_Run_Shell($)
{
   my ($string) = @_;
   my ($name, $cmd, @val) = split "\\|", $string;
   my $hash = $defs{$name};
...
} # end FB_FORK_Wlan_Run_Shell

#######################################################################
sub FB_FORK_Wlan_Run_Web($) 
{
   my ($string) = @_;
   my ($name, $cmd, @val) = split "\\|", $string;
   my $hash = $defs{$name};
...
# Set WLAN
   if ($hash->{WEBCM}) { #webcm
      ...
      FB_FORK_Log $hash, 3, "INFO: WEBCM Command.";
      FB_FORK_Web_CmdPost ($hash, \@webCmdArray);
      ...
      push @tr064CmdArray, ["WLANConfiguration:1", "wlanconfig1", "SetEnable", "NewEnable", $state]
               if $cmd =~ /^(wlan|wlan2\.4)$/;
      FB_FORK_Log $hash, 3, "INFO: TR-064 Command";
      $result = FB_FORK_TR064_Cmd( $hash, 0, \@tr064CmdArray );
   }
   else { #no API
      FB_FORK_Log $hash, 2, "ERROR: No API available to switch WLAN.";
...
} # end FB_FORK_Wlan_Run_Web
   
#######################################################################
sub FB_FORK_Ring_Run_Shell($)
{
   my ($string) = @_;
   my ($name, @val) = split "\\|", $string;
   my $hash = $defs{$name};
...
# Create a hash for the DECT devices whose ring tone (or radio station) can be changed
   foreach ( split( /,/, $intNo ) ) {
      if (defined $hash->{fhem}{$_}{brand} && "AVM" eq $hash->{fhem}{$_}{brand}) {
         FB_FORK_Log $hash, 5, "DEBUG: Internal number $_ seems to be a Fritz!Fon.";
 ...
 # Check if 2nd parameter is the duration
   ...
   if (int @val)
   {
      ...
      {
         $duration = $val[0];
         FB_FORK_Log $hash, 5, "DEBUG: Extracted ring duration of $duration s.";
         shift @val;
      }
   }
   ...
   if (int @val)
   {
      if ($val[0] !~ /^(msg|show|say|play):/i)
      {
         ...
         unless (defined ($ringTone)) {
            FB_FORK_Log $hash, 2, "ERROR: Ring tone '" . $val[0] . "' not valid";
            return $name."|0|Error: Ring tone '" . $val[0] . "' not valid";
         }
         FB_FORK_Log $hash, 5, "DEBUG: Ring tone $ringTone will be used.";
         shift @val;
      }
   }
...
# Create tts link to play as internet radio
   if ( $field{say} ) 
   {
      ...
      {
         FB_FORK_Log $hash, 2, "ERROR: Cannot do Text2Speech because box has no internet radio";
      }
   }
   ...
   if ($field{play})
   {
      ...
      {
        FB_FORK_Log $hash, 2, "ERROR: Cannot play mp3 because box has no internet radio";
      }
      elsif ($ttsLink)
      {
         FB_FORK_Log $hash, 4, "INFO: Ignore 'play:' because Text2Speech already defined.";
      }
      else
      {
         $ringTone = 33;
...
# Creation fhemRadioStation for ttsLink
   ...
   {
      FB_FORK_Log $hash, 3, "INFO: No Fritz!Fon identified, parameter 'say:' will be ignored."
   }
   elsif (int (@FritzFons) && $ttsLink && $hash->{fhem}{radio}{$fhemRadioStation} ne "FHEM")
   {
      FB_FORK_Log $hash, 3, "INFO: Create new internet radio station $fhemRadioStation: 'FHEM' for ringing with text-to-speech";
...
# Change ring tone of Fritz!Fons
   if ($ringTone)
   {
      FB_FORK_Log $hash, 3, "INFO: No Fritz!Fon identified, ring tone will be ignored."
         unless @FritzFons;
      foreach (@FritzFons)
      {
         ...
         FB_FORK_Log $hash, 4, "INFO: Change temporarily internal ring tone of Fritz!Fon DECT $_ to $ringTone";
         if ($ttsLink)
         {
            ...
            FB_FORK_Log $hash, 4, "INFO: Change temporarily radio station of Fritz!Fon DECT $_ to $fhemRadioStation (FHEM)";
         }
      }
   }
...
# uses name of port 0-3 (dial port 1-4) to show messages on ringing phone
   ...
   if ( $ringWithIntern =~ /^([1-3])$/ )
   {
      ...
      FB_FORK_Log $hash, 4, "INFO: Change temporarily name of calling number $ringWithIntern to '$msg'";
      push @cmdArray, "ctlmgr_ctl w telcfg settings/DialPort $ringWithIntern"
   } 
   elsif ($field{show})
   {
      FB_FORK_Log $hash, 3, "INFO: Parameter 'show:' ignored because attribute 'ringWithIntern' not defined."
   }
...
#Preparing 3rd command array to ring and reset everything
   FB_FORK_Log $hash, 4, "INFO: Ringing $intNo for $duration seconds";
...
} # End FB_FORK_Ring_Run_Shell

#######################################################################
sub FB_FORK_Ring_Run_Web($) 
{
   my ($string) = @_;
   my ($name, @val) = split "\\|", $string;
   my $hash = $defs{$name};
...
# Create a hash for the DECT devices whose ring tone (or radio station) can be changed
   foreach ( split( /,/, $intNo ) ) {
      if (defined $hash->{fhem}{$_}{brand} && "AVM" eq $hash->{fhem}{$_}{brand}) {
         ...
         FB_FORK_Log $hash, 5, "DEBUG: Internal number $_ (dect$userId) seems to be a Fritz!Fon.";
...
# no webcm no Message
   unless ( $hash->{WEBCM}==1 ) {
      $msg = "";
      FB_FORK_Log $hash, 3, "INFO: Your Fritz!OS version has limited interfaces. Parameter 'show:' ignored."
   ...
   if ( $field{say} ) {
      if ($fhemRadioStation) {
         ...
         # if ($ttsRessource eq "ESpeak"){
             ...
             # FB_FORK_Log $hash, 4, "INFO: Text2Speech:" .$cmd;
             # system($cmd);
         # }
      ...
      # speak with Translate.Google 
         ...
         FB_FORK_Log $hash, 2, "INFO: Cannot do Text2Speech because box has no internet radio";
      }
   }
...
# Extract play link
   if ( $field{play} ) {
      unless ($fhemRadioStation)
      {
        FB_FORK_Log $hash, 3, "INFO: Cannot play mp3 because box has no internet radio";
      }
      elsif ($ttsLink)
      {
         FB_FORK_Log $hash, 3, "INFO: Ignore 'play:' because Text2Speech already defined.";
...
# Store current values for fon and dect port
   ...
   FB_FORK_Log $hash, 4, "INFO: Read current dect and fon port values from box";
   $startValue = FB_FORK_Web_Query( $hash, $queryStr, 'UTF-8') ;
   ...
   unless ($startValue->{useClickToDial}) {
      if ($hash->{WEBCM}) { # mit webcm
         ...
         $startValue->{dialPort}=50;
           FB_FORK_Log $hash, 3, "INFO: Switch ClickToDial on, set dial port 50";
      }
      elsif ($hash->{SECPORT}) { # oder mit TR064
         # get port name
         push @tr064CmdArray, ["X_VoIP:1", "x_voip", "X_AVM-DE_GetPhonePort", "NewIndex", "1"];
         @tr064Result = FB_FORK_TR064_Cmd( $hash, 0, \@tr064CmdArray );
         ...
         if ($portName) {
            ...
            @tr064Result = FB_FORK_TR064_Cmd( $hash, 0, \@tr064CmdArray );
            FB_FORK_Log $hash, 3, "INFO: Switch ClickToDial on, set dial port '$portName'";
         }
      }
...
# Creation fhemRadioStation for ttsLink
      ...
      if (int @webCmdArray) {
         ...
         FB_FORK_Log $hash, 3, "INFO: Your Fritz!OS version has limited interfaces. Cannot create radio station 'FHEM' for ringing with text-to-speech."
      }
   }
   
   #Execute command array
   FB_FORK_Web_CmdPost( $hash, \@webCmdArray )       if int @webCmdArray;
   
#Preparing 2nd command array to set ring parameters
# Change ring tone of Fritz!Fons
   if ( $ringTone && $hash->{WEBCM}==0 && $useGuiHack == 0 ) {
      FB_FORK_Log $hash, 3, "INFO: Your Fritz!OS version has limited interfaces. Ring tone cannot be changed."
   }
      ...
      foreach (@FritzFons) {
         ...
         FB_FORK_Log $hash, 4, "INFO: Change temporarily internal ring tone of dect".$_." to $ringTone";
         if ($ttsLink) {
            ...
            FB_FORK_Log $hash, 4, "INFO: Change temporarily radio station of dect".$_." to $fhemRadioStation (FHEM)";
         }
   ...
   if ( $ringWithIntern =~ /^([1-3])$/ && $msg) {
      if ($startValue->{fonPort}->[$ringWithIntern-1]->{Name}) {
         push @webCmdArray, "telcfg:settings/MSN/Port".($ringWithIntern-1)."/Name" => $msg;
            FB_FORK_Log $hash, 4, "INFO: Change temporarily name of dial port 'fon$ringWithIntern' to '$msg'";
      }
      else {
         FB_FORK_Log $hash, 2, "ERROR: Current name of dial port 'fon$ringWithIntern' could not be determined -> Did not change the name.";
         my $temp = Dumper( $startValue );
         FB_FORK_Log $hash, 3, "DEBUG: info: \n".$temp;
      }
      push @webCmdArray, "telcfg:settings/DialPort" => $ringWithIntern;
         FB_FORK_Log $hash, 4, "INFO: Set dial port to '" . $dialPort{$ringWithIntern} . "' (MSN: ".$startValue->{fonPort}->[$ringWithIntern-1]{MSN} .").";
   } 
# set dial port to 50 (all Fons)
      ...
      FB_FORK_Log $hash, 3, "INFO: Parameter 'show:' ignored because attribute 'ringWithIntern' not defined."
            if $field{show};
      push @webCmdArray, "telcfg:settings/DialPort" => 50;
         FB_FORK_Log $hash, 4, "INFO: Set dial port to 50 (all fons)";
   }
...
#Execute command array
   ...
   $result = FB_FORK_Web_CmdGet( $hash, \@getCmdArray )
...
#Preparing 3rd command array to ring
   ...
   if ( $hash->{WEBCM}==1 ) {
      ...
      FB_FORK_Log $hash, 3, "INFO: Your Fritz!OS version has limited interfaces. You cannot ring.";
   }
...
#Preparing 4th command array to stop ringing (but not when duration is 0 or play: and say: is used without duration)
   unless ( $duration == 0 || $duration == -1 && $ttsLink ) {
      push @tr064CmdArray, ["X_VoIP:1", "x_voip", "X_AVM-DE_DialHangup"];
      $result = FB_FORK_TR064_Cmd( $hash, 0, \@tr064CmdArray )      if $hash->{SECPORT} && $hash->{WEBCM} != 1;
...
#Preparing 5th command array to reset everything
   push @webCmdArray, "telcfg:settings/DialPort" => $startValue->{dialPort}      if defined $startValue->{dialPort};
      FB_FORK_Log $hash, 4, "INFO: Reset dial port to '".$dialPort{$startValue->{dialPort}}."'.";
# Reset internal ring tones for the Fritz!Fons
   if ($ringTone) {
      foreach (@FritzFons) {
         ...
         my $getCmdStr = "&ring_tone_radio_test=1&idx=".$_."&start_ringtest=1&ringtone=".$value;
            FB_FORK_Log $hash, 4, "INFO: Reset ring tone of dect$_ to $value";
         # Reset internet station for the Fritz!Fons
         if ($ttsLink) {
            ...
            FB_FORK_Log $hash, 4, "INFO: Reset radio station of dect$_ to $value";
         }
         push @getCmdArray, [ "fon_devices/edit_dect_ring_tone.lua" => $getCmdStr ] 
               if $useGuiHack ;
      }
   ...
   if ($ringWithIntern =~ /^([1-2])$/) {
      ...
      if ($fonName) {# darf nie leer sein
         push( @webCmdArray, "telcfg:settings/MSN/Port".($ringWithIntern-1)."/Name" => $fonName ) ; 
            FB_FORK_Log $hash, 4, "INFO: Reset name of dial port fon$ringWithIntern to '$fonName'";
      }
   }
...
# ??? Switch of Internet Radio stations 
   ...
   # }
#set FB_FORK ring 612 show:test test say:test test
...
# Execute command array
   ...
   if ( $result->[0] == 1 ) {
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->sid", $result->[1];
      FB_FORK_Readout_Add_Reading $hash, \@roReadings, "fhem->sidTime", time();
   }
   FB_FORK_Readout_Add_Reading $hash, \@roReadings, "readoutTime", sprintf( "%.2f", time()-$startTime);
   ...
   # return $name."|1|Ringing done";
} # End FB_FORK_Ring_Run_Web

#######################################################################
sub FB_FORK_Set_Alarm_Shell($@)
{
   my ($hash, @val) = @_;
   my $name = $hash->{NAME};
   ...
   my $result = FB_FORK_Telnet_OpenCon( $hash );
   return "$name|Error|$result"
      if $result;

   readingsBeginUpdate($hash);

   FB_FORK_Shell_Exec( $hash, "ctlmgr_ctl w telcfg settings/AlarmClock".($alarm-1)."/Active ".$state );
   readingsBulkUpdate($hash,"alarm".$alarm."_state",$stateTxt);

   if (defined $time)
   {
      FB_FORK_Shell_Exec( $hash, "ctlmgr_ctl w telcfg settings/AlarmClock".($alarm-1)."/Time ".$time );
      readingsBulkUpdate($hash,"alarm".$alarm."_time",$timeTxt);
   }

   if (defined $day)
   {
      FB_FORK_Shell_Exec( $hash, "ctlmgr_ctl w telcfg settings/AlarmClock".($alarm-1)."/Weekdays ".$day );
      chop $dayTxt;
   ...
   return undef;
} # end FB_FORK_Set_Alarm_Shell
   
#######################################################################
sub FB_FORK_Set_Alarm_Web($@)
{
   my ($hash, @val) = @_;
   my $name = $hash->{NAME};
   my @webCmdArray;
...
{  
   ...
   if (lc $type eq lc $mohtype{0} || $type eq "0") {
      FB_FORK_Shell_Exec ($hash, 'ctlmgr_ctl w telcfg settings/MOHType 0');
      return 0;
   }
   elsif (lc $type eq lc $mohtype{1} || $type eq "1") {
      FB_FORK_Shell_Exec ($hash, 'ctlmgr_ctl w telcfg settings/MOHType 1');
      return 1;
   }
...
# Load customer MOH file
   ...
   if ($inFile =~ /^say:/i)
   {
      FB_FORK_Log $hash, 4, "INFO: Converting Text2Speech";
...
{  
   ...
   $outFile = substr($inFile,0,-4)
      if ($inFile =~ /\.(mp3|wav)$/i);
   my $returnStr = FB_FORK_Shell_Exec ($hash
      , 'ffmpegconv -i "'.$inFile.'" -o "'.$outFile.'.moh" --limit 32 --type 6');
   return $returnStr;
} # end FB_FORK_ConvertMOH

#######################################################################
sub FB_FORK_ConvertRingTone ($@)
{  
   my ($hash, @file) = @_;
...
# Get current ringtone
   ...
   my $startValue = FB_FORK_Web_Query( $hash, $queryStr );
   
# Set ring tone Internet Radio
      FB_FORK_Log $hash, 5, "DEBUG: Set ring tone of $intNo to radio $radioStation";
   ...
   FB_FORK_Web_CmdPost( $hash, \@webCmdArray );
      ...
      FB_FORK_Log $hash, 5, "DEBUG: Call $intNo";
...
# Reset ring tone
      FB_FORK_Log $hash, 5, "DEBUG: Reset ring tones.";
   ...
   return undef;
} # END sub FB_FORK_StartRadio_Web
...
sub FB_FORK_Shell_Query($$$)
{
   ...
   my $resultArray = FB_FORK_Shell_Exec( $hash, \@cmdArray);
...
sub FB_FORK_Shell_Exec($$)
{
   ...
   if ($hash->{REMOTE} == 1) {
      unless (defined $telnet) {
         return undef
            if (FB_FORK_Telnet_OpenCon($hash));
         $openedTelnet = 1;
      }
      my $retVal = FB_FORK_Shell_Exec_Telnet($hash, $cmd);
      FB_FORK_Telnet_CloseCon ( $hash ) if $openedTelnet;
      return $retVal;
   }
   else {
      return FB_FORK_Shell_Exec_Local($hash, $cmd);
   }
...
############################################
sub FB_FORK_Shell_Exec_Local($$)
{
   my ($hash, $cmd) = @_;
  
   if (ref \$cmd eq "SCALAR") {
      FB_FORK_Log $hash, 5, "DEBUG: Execute '".$cmd."'";
      my $result = qx($cmd);
      chomp $result;
      FB_FORK_Log $hash, 5, "DEBUG: Result '$result'";
      return $result;
   }
   elsif (ref \$cmd eq "REF") {
      if ( int (@{$cmd}) > 0 )
      {
         FB_FORK_Log $hash, 4, "INFO: Execute " . int ( @{$cmd} ) . " command(s)";
         FB_FORK_Log $hash, 5, "DEBUG: Commands: '" . join( " | ", @{$cmd} ) . "'";
         my $cmdStr = join "\necho ' |#|'\n", @{$cmd};
         $cmdStr .= "\necho ' |#|'";
         my $result = qx($cmdStr);
         unless (defined $result)
         {
            FB_FORK_Log $hash, 1, "ERROR: No STDOUT from shell command.";
            return undef;
         }
         $result =~ s/\n|\r//g;
         ...
         @{$cmd} = ();
         FB_FORK_Log $hash, 4, "INFO: Received ".int(@resultArray)." answer(s)";
         FB_FORK_Log $hash, 5, "DEBUG: Result: '" . join (" | ", @resultArray)."'";
         return \@resultArray;
      }
      else
      {
         FB_FORK_Log $hash, 4, "INFO: No shell command to execute.";
      }
   }
   else {
      FB_FORK_Log $hash, 1, "ERROR: wrong perl parameter";
   }
}

# Executed a command via Telnet
############################################
sub FB_FORK_Shell_Exec_Telnet($$)
{
   my ($hash, $cmd) = @_;
   my @output;
   my $result;
   ...
   if (ref \$cmd eq "SCALAR") {
      FB_FORK_Log $hash, 4, "INFO: Execute '".$cmd."'";
      @output=$telnet->cmd($cmd);
      $result = $output[0];
      chomp $result;
      my $log = join " ", @output;
      chomp $log;
      FB_FORK_Log $hash, 4, "INFO: Result '$log'";
      return $result;
   }
   elsif (ref \$cmd eq "REF") {
      my @resultArray = ();
      if ( int (@{$cmd}) > 0 )
      {
         ...
         foreach (@{$cmd})
         {
            FB_FORK_Log $hash, 5, "DEBUG: Execute '$_'";
            unless ($_ =~ /^sleep/)
            {
               @output=$telnet->cmd($_);
               $result = $output[0] || "";
               ...
               my $log = join "", @output;
               chomp $log;
               FB_FORK_Log $hash, 5, "DEBUG: Result '$log'";
            }
            else
            {
               FB_FORK_Log $hash, 4, "INFO: Do '$_' in perl.";
               eval ($_);
               $result = "";
            }
...
sub FB_FORK_Telnet_OpenCon($)
{
   ...
   my $pwd = FB_FORK_readPassword($hash);
   my $msg;
   my $before;
   my $match;
   ...
   unless (defined $pwd) {
      ...
      FB_FORK_Log $hash, 5, "DEBUG: Open password file '$pwdFile' to extract password";
      if (open(IN, "<" . $pwdFile)) {
         $pwd = <IN>;
         close(IN);
        FB_FORK_Log $hash, 5, "DEBUG: Close password file";
      } else {
         FB_FORK_Log $hash, 2, $msg;
         return $msg;
      }
   ...
   my $user = AttrVal( $name, "telnetUser", "" );
      ...
      FB_FORK_Log $hash, 4, "INFO: Open Telnet connection to $host";
   my $timeout = AttrVal( $name, "telnetTimeOut", "10");
   ...
   FB_FORK_Log $hash, 5, "DEBUG: Wait for user or password prompt.";
   ...
   if ( $match =~ /(user|login): / && $user eq "") {
      ...
      FB_FORK_Log $hash, 5, "DEBUG: Entering user name";
      $telnet->print( $user );

      FB_FORK_Log $hash, 5, "DEBUG: Wait for password prompt";
      ...
      FB_FORK_Log $hash, 3, "INFO: Attribute 'telnetUser' defined but telnet login did not prompt for user name.";
   }

      FB_FORK_Log $hash, 5, "DEBUG: Entering password";
   $telnet->print( $pwd );

      FB_FORK_Log $hash, 5, "DEBUG: Wait for command prompt";
...
# redirect console messages
   $telnet->cmd("setconsole -r");
      ...
      FB_FORK_Log $hash, 5, "DEBUG: Change command prompt";
   ...
   return undef;
} # end FB_FORK_Telnet_OpenCon
   
# Closes a Telnet Connection to an external FritzBox
############################################
sub FB_FORK_Telnet_CloseCon($)
{
   my ($hash) = @_;
   ...
   if (defined $telnet) {
      FB_FORK_Log $hash, 4, "INFO: Close Telnet connection";
      $telnet->close;
      $telnet = undef;
   }
   else {
      FB_FORK_Log $hash, 1, "INFO: Cannot close an undefined Telnet connection";
   }
} # end FB_FORK_Telnet_CloseCon

# Execute a Command via TR-064
#################################################
sub FB_FORK_TR064_Cmd($$$)
{
   ...
   unless ($port) {
      FB_FORK_Log $hash, 2, "ERROR: TR064 not used. No security port defined.";
      return undef;
   }

# Set Password und User for TR064 access
   $FB_FORK_TR064pwd = FB_FORK_readPassword($hash)     unless defined $FB_FORK_TR064pwd;
   $FB_FORK_TR064user = AttrVal( $name, "boxUser", "dslf-config" );   
   
   my $host = $hash->{HOST};

   FB_FORK_Log $hash, 5, "DEBUG: Credentials: " . $host . " / " . $FB_FORK_TR064user . " / " . $FB_FORK_TR064pwd;
   ...
   # Prepare action parameter
      ...
      FB_FORK_Log $hash, 4, "INFO: Perform TR-064 call - " . $logMsg;
      ...
      unless( $res ) { # Transport-Error
         FB_FORK_Log $hash, 2, "ERROR: TR064-Transport-Error: ".$soap->transport->status;
         my %errorMsg = ( "Error" => $soap->transport->status );
         push @retArray, \%errorMsg;
         $FB_FORK_TR064pwd = undef;
      }
         ...
         FB_FORK_Log $hash, 2, "ERROR: TR064 error $ecode:$edesc ($logMsg)";
         @{$cmdArray} = ();
...
} # End of FB_FORK_TR064_Cmd

# get FB_FORK tr064ServiceList
#################################################
sub FB_FORK_TR064_Get_ServiceList($)
{
   my ($hash) = @_;
   my $name = $defs{NAME};
   ...
   FB_FORK_Log $hash, 5, "DEBUG: Getting service page $url";
   my $agent    = LWP::UserAgent->new( env_proxy => 1, keep_alive => 1, protocols_allowed => ['http'], timeout => 10);
...
# Get actions of each service
   foreach (@serviceArray) {
      ...
      $url = 'http://'.$host.":49000".$_->[2];
         ...
         FB_FORK_Log $hash, 5, "DEBUG: Getting action page $url";
      my $agent    = LWP::UserAgent->new( env_proxy => 1, keep_alive => 1, protocols_allowed => ['http'], timeout => 10);
      my $response = $agent->get( $url );
...
sub FB_FORK_TR064_Init ($$)
{
   ...
   if ($missingModulTR064) {
      FB_FORK_Log $hash, 2,  "ERROR: Cannot use TR-064. Perl modul ".$missingModulTR064."is missing on this system. Please install.";
      return undef;
   }

# Security Port anfordern
      FB_FORK_Log $hash, 4, "INFO: Open TR-064 connection and ask for security port";
   my $s = SOAP::Lite
      -> uri('urn:dslforum-org:service:DeviceInfo:1')
      -> proxy('http://'.$host.':49000/upnp/control/deviceinfo', timeout => 10 )
      -> getSecurityPort();

   FB_FORK_Log $hash, 4, "INFO: SecPort-String " . Dumper($s);

   my $port = $s->result;
   FB_FORK_Log $hash, 4, "INFO: SecPort-Result " . Dumper($s->result);

   unless( $port ) {
      FB_FORK_Log $hash, 2, "ERROR: Could not get secure port: $!";
      return undef;
   }
...
#   $hash->{TR064USER} = "dslf-config";
   ...
   sub SOAP::Transport::HTTP::Client::get_basic_credentials {return  $FB_FORK_TR064user => $FB_FORK_TR064pwd;}
...
sub FB_FORK_Web_OpenCon ($)
{
   ...
   if ($missingModulWeb) {
      FB_FORK_Log $hash, 2, "ERROR: Perl modul ".$missingModulWeb."is missing on this system. Please install before using this modul.";
      return undef;
   }
...
# Use old sid if last access later than 9.5 minutes
   ...
   if (defined $sid && $hash->{fhem}{sidTime} > time() - 9.5 * 60) {
      FB_FORK_Log $hash, 4, "INFO: using old SID from " . $hash->{fhem}{sidTime};
      return $sid
   }
   ...
   unless (defined $pwd) {
      FB_FORK_Log $hash, 2, "ERROR: No password set. Please define it (once) with 'set $name password YourPassword'";
      return undef;
   }
   my $user = AttrVal( $name, "boxUser", "" );
   $user = AttrVal( $name, "telnetUser", "" ) if $user eq "";

   FB_FORK_Log $hash, 4, "INFO: Open Web connection to $host";
   FB_FORK_Log $hash, 4, "INFO: getting new SID";
   $sid = (FB_doCheckPW($host, $user, $pwd));
   
   if ($sid) {
      FB_FORK_Log $hash, 4, "INFO: Web session opened with sid $sid";
      return $sid;
   } 
   
   FB_FORK_Log $hash, 2, "ERROR: Web connection could not be established. Please check your credentials (password, user).";
   return undef;
...
sub FB_FORK_Web_CmdPost($$@)
{
   ...
   my $sid = FB_FORK_Web_OpenCon($hash);
   unless ($sid) {
...
# Complete the arguments
   ...
   FB_FORK_Log $hash, 3, "INFO: Posting ".(@{$webCmdArray} /2) ." parameters to '$url'";
   my $agent    = LWP::UserAgent->new( env_proxy => 1, keep_alive => 1, protocols_allowed => ['http'], timeout => 10);
   ...
   FB_FORK_Log $hash, 3, "INFO: Response: ".$response->content;
# if ($response->content)
   # {
...
sub FB_FORK_Web_CmdGet($$)
{
   ...
   foreach ( @{$getCmdArray} ) {
      ...
      FB_FORK_Log $hash, 4, "INFO: Execute HTTP-Get '$url'";
      my $response = $agent->get( $url );
      ...
      if ($response->is_error) {
         ...
         FB_FORK_Log $hash, 5, "DEBUG: Response: ".$response->content();
      }
   }
...
sub FB_FORK_Web_Query($$@)
{
   ...
   my $sid = FB_FORK_Web_OpenCon( $hash );
   unless ($sid) {
      FB_FORK_Log $hash, 2, "ERROR: Didn't get a session ID. ResetSID";
      my %retHash = ( "Error" => "Didn't get a session ID", "ResetSID" => "1" ) ;
      return \%retHash;
   }

   FB_FORK_Log $hash, 5, "DEBUG: Request data via API " . $f_lua;
   my $host = $hash->{HOST};
   my $url = 'http://' . $host;
   ...
   if ( $f_lua eq "luaQuery") {
     ...
     FB_FORK_Log $hash, 2, "ERROR: Wrong function name. function_name: " . $f_lua;
   ...
   FB_FORK_Log $hash, 5, "get -> URL: $url";

   $response = $agent->get ( $url );

   FB_FORK_Log $hash, 5, "DEBUG: Response: " . $response->status_line . "\n" . $response->content;
...
#################
   ...
   if ( $f_lua ne "luaCall") {
     ...
     return FB_FORK_Process_JSON($hash, $response->content, $sid, $charSet);
...
sub FB_FORK_Lua_Data($$@)
{
   ...
   unless ($sid) {
      my %retHash = ( "Error" => "Didn't get a session ID", "ResetSID" => "1" ) ;
      FB_FORK_Log $hash, 2, "ERROR: Didn't get a session ID";
      return \%retHash;
   }

   FB_FORK_Log $hash, $LogInfo, "INFO: Request data via API dataQuery.";
   my $host = $hash->{HOST};
   my $url = 'http://' . $host . '/data.lua?sid=' . $sid;

   FB_FORK_Log $hash, $LogInfo, "INFO: URL: $url";
  
   my $agent    = LWP::UserAgent->new( env_proxy => 1, keep_alive => 1, protocols_allowed => ['http'], timeout => 180);
   my $response = $agent->post ( $url, $queryStr );

   FB_FORK_Log $hash, $LogInfo, "INFO: Response: " . $response->status_line . "\n" . $response->content;
   ...
   if ( $data =~ m/\<table id="uiProfileList"(.*?)\<\/table\>/is ) {
     ...
     FB_FORK_Log $hash, $LogInfo, "INFO: Response 1: " . $1;

     my $count = 0;

     foreach my $line ($data =~ m/$pattern_tr/gs) {
       FB_FORK_Log $hash, $LogInfo, "INFO: Response 2: " . $line;

       if ($line =~ m/$pattern_vl/gs) {
         FB_FORK_Log $hash, $LogInfo, "INFO: Profile name: " . $1 . "Profile Id: " . $2;
...
#################
     FB_FORK_Log $hash, $LogInfo, "INFO: Response 1: " . $profile_content;
#################
  
     return FB_FORK_Process_JSON($hash, $profile_content, $sid, $charSet);
   }

#################
   FB_FORK_Log $hash, $LogInfo, "INFO: Response: " . $response->content;
#################
...
sub FB_FORK_Process_JSON($$$@) {
   ...
   if ($jsonText =~ /<html|"pid": "logout"|<head>/) {
      FB_FORK_Log $hash, 5, "DEBUG: Old SID not valid anymore. ResetSID";
   ...
   FB_FORK_Log $hash, 5, "DEBUG: Decode JSON string.";

   my $jsonResult ;
   if ($charSet eq "UTF-8") {
      $jsonResult = eval { JSON->new->utf8->decode( $jsonText ) };
      if ($@) {
        FB_FORK_Log $hash, 2, "INFO: Decode JSON string: decode_json failed, invalid json. error:$@";
      }
   } 
   else {
   ...
   # FB_FORK_Log $hash, 5, "DEBUG: JSON: " . Dumper($jsonResult);
   
   #Not a HASH reference at ./FHEM/72_FB_FORK.pm line 4662.
   # 2018.03.19 18:43:28 3: FB_FORK: get FB_FORK luaQuery settings/sip
   if ( ref ($jsonResult) ne "HASH" ) {
      chop $jsonText;
      FB_FORK_Log $hash, 2, "ERROR: no json string returned (" . $jsonText . ")";
...
sub FB_FORK_User_Info_List($) {
   ...
   if ( defined $result->{Error} ) {
     my $tmp = FB_FORK_ERR_Result($hash, $result);
     FB_FORK_Log $hash, 2, "ERROR: evaluating user info -> " . $tmp;
     return $returnStr . $tmp;
   } elsif ( defined $result->{AuthorizationRequired} ) {
     FB_FORK_Log $hash, 2, "ERROR: evaluating user info -> AuthorizationRequired";
     return $returnStr . "AuthorizationRequired";
   }

   eval {
      FB_FORK_Log $hash, 5, "DEBUG: evaluating user info: \n" . Dumper $result->{user_info};
   };
...
sub FB_FORK_VPN_Shares_List($) {
   ...
   eval {
     foreach my $key (keys %$views) {
       FB_FORK_Log $hash, 4, "INFO: userConnections: ".$key;
       $returnStr .= "<tr>\n";
       $returnStr .= "<td>" . $key . "</td>";
   ...
   eval {
     foreach my $key (keys %$views) {
       FB_FORK_Log $hash, 4, "INFO: boxConnections: ".$key;
       $returnStr .= "<tr>\n";
       $returnStr .= "<td>" . $key . "</td>";
   ...
   #FB_FORK_Log $hash, 3, "INFO: active->keys: ".$nbViews;
   #FB_FORK_Log $hash, 3, "INFO: active->status: ".$a_test;
   #FB_FORK_Log $hash, 3, "INFO: passive->status: ".$p_test;
...
############################################
sub FB_FORK_Lan_Device_Info($$$) {
   my ($hash, $lDevID, $action) = @_;
   my $name = $hash->{NAME};
   FB_FORK_Log $hash, 4, "INFO: LanDevice to proof: " . $lDevID . " for: " . $action;
   ...
   FB_FORK_Log $hash, 4, "INFO: FB_FORK_Lan_Device_Info f�r Version: $FW1.$FW2 ";
   ...
   FB_FORK_Log $hash, 4, "INFO: set $name $action " . join(" ", @webCmdArray);
   ...
   FB_FORK_Log $hash, 5, "DEBUG: get $name $action " . $queryStr;

   my $result = FB_FORK_Lua_Data( $hash, \@webCmdArray) ;

   if(defined $result->{Error}) {
      FB_FORK_Log $hash, 2, "ERROR: get $name $action " . $queryStr;
      return "ERROR: holen Lan_Device_Info: " . $action . " f�r: " . $lDevID;
   }

   if (exists $result->{data}->{vars}) {
     FB_FORK_Log $hash, 4, "INFO: landevice: " . $lDevID . "landevice: " . $result->{data}->{vars}->{dev}->{UID};
     ...
     if ($action eq "info") {
       ...
       unless (defined $result->{data}->{vars}->{dev}->{netAccess}->{kisi}->{selectedRights}->{msgid}) {
         FB_FORK_Log $hash, 2, "ERROR: no msgId returned";
         return "ERROR: no msgId returned";
       }

       my $jsonMsgId = $result->{data}->{vars}->{dev}->{netAccess}->{kisi}->{selectedRights}->{msgid};
 
       FB_FORK_Log $hash, 5, "DEBUG: MsgId: " . $jsonMsgId;
       return "INFO: " . $jsonMsgId;
     }
   } else {
     FB_FORK_Log $hash, 2, "ERROR: landevice: " . $lDevID . "landevice: Fehler holen Lan_Device_Info";
 
     return "ERROR: Lan_Device_Info: " . $action . " f�r: " . $lDevID;
   }
}

sub FB_FORK_Lua_Kids($$@)
{
   my ($hash, $queryStr, $charSet) = @_;
   $charSet = "" unless defined $charSet;
   my $name = $hash->{NAME};
   ...
   FB_FORK_Log $hash, 5, "DEBUG: Request data via API dataQuery.";
   my $host = $hash->{HOST};
   my $url = 'http://' . $host . '/internet/kids_userlist.lua?sid=' . $sid; # . '&' . $queryStr;

   FB_FORK_Log $hash, 5, "DEBUG: URL: $url";
   ...
   if ($jsonText =~ /<html>|"pid": "logout"/) {
      FB_FORK_Log $hash, 2, "ERROR: Old SID not valid anymore. ResetSID";
      my %retHash = ("Error" => "Old SID not valid anymore.", "ResetSID" => "1");
      return \%retHash;
   }
...
#################
  ...
  # 2018.03.19 18:43:28 3: FB_FORK: get FB_FORK luaQuery settings/sip
   if ( ref ($jsonResult) ne "HASH" ) {
      chop $jsonText;
      FB_FORK_Log $hash, 5, "DEBUG: no json string returned (" . $jsonText . ")";
      my %retHash = ("Error" => "no json string returned (" . $jsonText . ")", "ResetSID" => "1");
      return \%retHash;
   }
   $jsonResult->{sid} = $sid;
...
} # end FB_FORK_storePassword
   
#####################################
# reads the FritzBox password
sub FB_FORK_readPassword($)
{
   my ($hash) = @_;
   my $name = $hash->{NAME};
   ...
   FB_FORK_Log $hash, 4, "INFO: Read FritzBox password from file";
   ($err, $password) = getKeyValue($index);

   if ( defined($err) ) {
      FB_FORK_Log $hash, 2, "ERROR: unable to read FritzBox password from file: $err";
      return undef;
   }  
   ...
   if ( defined($password) ) {
      ...
      FB_FORK_Log $hash, 2, "ERROR: No password in file";
      return undef;
   }
} # end FB_FORK_readPassword
   
##################################### 
sub FB_FORK_fritztris($)
{
  my ($d) = @_;
  $d = "<none>" if(!$d);
  return "$d is not a FB_FORK instance<br>"
        if(!$defs{$d} || $defs{$d}{TYPE} ne "FB_FORK");
...
<ul>
   ...
   <ul>
      <br>
      <code>define &lt;name&gt; FB_FORK  [host]</code>
      <br/>
      ...
      <br/><br/>
      Example: <code>define FritzBox FB_FORK</code>
      <br/><br/>
      The FritzOS has a hidden function (easter egg).
      <br>
      <code>define MyEasterEgg weblink htmlCode { FB_FORK_fritztris("FritzBox") }</code>
      <br/><br/>
   </ul>
   ...
   <ul>
      ...
      <li><a name="dectRingblock"></a>
         ...
         lmode:on|off = lmode definiert die Sperre. Bei off ist sie aus, außer f�r den angegebenen Zeitraum.<br>
                                                    Bei on ist die Sperre an, außer f�r den angegebenen Zeitraum<br>
   ...
   <ul>
      ...
      <li><a name="luaInfo"></a>
         ...
         userInfos -> Shows a list of FritzBox users.<br>
      </li><br>
...
<ul>
   ...
   <ul>
      ...
      <br/><br/>
      Beispiel: <code>define Fritzbox FB_FORK</code>
      <br/><br/>
   ...
   <ul>
      ...
      <li><a name="macFilter"></a>
         <dt><code>set &lt;name&gt; macFilter &lt;on|off&gt;</code></dt>
         <br>
         Schaltet den MAC Filter an oder aus. In der FritzBox unter "neue WLAN Geräte zulassen/sperren<br>
         Benötigt FRITZ!OS 7.21 oder höher.
      </li><br>
   ...
   <ul>
      ...
      <li><a name="luaInfo"></a>
         ...
         userInfos -> Generiert eine Liste der FritzBox Benutzer.<br>
      </li><br>
