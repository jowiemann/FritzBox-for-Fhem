###############################################################
# $Id: 72_FRITZBOX_2.pm 17437 2023-01-05 18:30:00Z fork $
#
#  72_FRITZBOX_2.pm 
#
...
my $ModulVersion = "FB-Fork 0.2.13";
my $missingModul = "";
   ...
   if ( $host =~ /undefined|local/ ) {
      ...
      if ( -X "/usr/bin/ctlmgr_ctl" ) {
         if ( $< != 0 ) {
            ...
            FRITZBOX_Log $hash, 5, "INFO: Fhem is running on a Fritz!Box as 'root' user.";
         }
      }
   }
...
# Determine local or remote mode
   if ($fritzShell) {
      ...
      FRITZBOX_Log $hash, 4, "INFO: FRITZBOX modul runs in local mode.";
   }
   else {
      FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "->REMOTE", 1;
      FRITZBOX_Log $hash, 4, "INFO: FRITZBOX modul runs in remote mode.";
   }
...
# Check for remote APIs
   ...
   # Check if webcm exists
      ...
      if ($response->is_success) {
         ...
         FRITZBOX_Log $hash, 4, "INFO: API webcm found (".$response->code.").";
      }
      else {
         FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "->WEBCM", 0;
         FRITZBOX_Log $hash, 4, "INFO: API webcm does not exist (".$response->status_line.")";
      }
   ...
   # Check if query.lua exists
      ...
      if ($response->is_success) {
         ...
         FRITZBOX_Log $hash, 4, "INFO: API luaQuery found (".$response->code.").";
      }
      elsif ($response->code eq "500" || $response->code eq "403") {
         FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "->LUAQUERY", 1;
         FRITZBOX_Log $hash, 4, "INFO: API luaQuery found but responded with: ".$response->status_line;
      }
      else {
         FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "->LUAQUERY", 0;
         FRITZBOX_Log $hash, 4, "INFO: API luaQuery does not exist (".$response->status_line.")";
      }
   ...
   # Check if tr064 specification exists and determine TR064-Port
      ...
      if ($response->is_success) { #determine TR064-Port
         ...
         FRITZBOX_Log $hash, 4, "INFO: API TR-064 found.";
      #Determine TR064-Port
         ...
         if ($tr064Port) {
            ...
            FRITZBOX_Log $hash, 4, "INFO: TR-064-SecurePort is $tr064Port.";
         }
         else {
            FRITZBOX_Log $hash, 4, "INFO: TR-064-SecurePort does not exist";
         }
      }
      else {
   ...
   # Check if m3u can be created and the URL tested
      ...
      if (open my $fh, '>', $m3uFileLocal) {
         ...
         print $fh $ttsLink;
         close $fh;
         FRITZBOX_Log $hash, 4, "INFO: Created m3u file '$m3uFileLocal'.";
      ...
      # if no URL and no local file defined, then try to build the correct URL 
         ...
         # Getting IP of FHEM host
            FRITZBOX_Log $hash, 4, "INFO: Try to get my IP address.";
            ...
            $ip = $socket->sockhost if $socket; #A side-effect of making a socket connection is that our IP address is available from the 'sockhost' method
               FRITZBOX_Log $hash, 4, "INFO: Could not determine my ip address"  unless $ip;
         # Get a web port
            my $port;
               FRITZBOX_Log $hash, 4, "INFO: Try to get a FHEMWEB port.";
            foreach( keys %defs ) {
            ...
            FRITZBOX_Log $hash, 4, "INFO: Could not find a FHEMWEB device."  unless $port;
      ...
      # Check if m3u can be accessed
         unless ( $m3uFileURL eq "unknown" ) {
            FRITZBOX_Log $hash, 4, "INFO: Try to get '$m3uFileURL'";
            $response = $agent->get( $m3uFileURL );
            if ($response->is_error) {
               FRITZBOX_Log $hash, 4, "INFO: Failed to get '$m3uFileURL': ".$response->status_line;
               $m3uFileURL = "unknown"     ;
            }
         }
   ...
   # Box model per jason
      ...
      # FRITZBOX_Log $hash, 5, "INFO: jason_boxinfo returned: $content";
    ...
    # Ansonsten Box-Model per system_status einlesen
      ...
      # Muss nochmal neu gesetzt werden, sonst gibt es einen Fehler (keine Ahnung warum)
         ...
         FRITZBOX_Log $hash, 5, "INFO: Read 'system_status'";
         
         $response = $agent->get( $url );
         $content  = $response->content;
         # FRITZBOX_Log $hash, 5, "INFO: system_status returned: $content";
         if ($response->is_success) {
...
# Check if telnet modul exists
   if ($missingModulTelnet) {
      FRITZBOX_Log $hash, 4, "INFO: Cannot check for telnet access because perl modul $missingModulTelnet is missing on this system.\n";
   }
   else {
      ...
      if (!$telnet) {
         ...
         $telnet = undef;
         FRITZBOX_Log $hash, 4, "INFO: Could not open telnet connection to $host: $!";
      }   
      else {
   ...
   FRITZBOX_Log $hash, 5, "INFO: Handover to main process (".length ($returnStr)."): ".$returnStr;
   ...
   # Handset and DECT user can be in different orders
      for (0..$handsetCount) {
         ...
         if ($dectUserID) {
            ...
            FRITZBOX_Log $hash, 5, "INFO: dect".$dectUser."_manufacturer: " . $resultArray->[ $offset + 1 ];
            push @roReadings, "dect".$dectUser."_model|" . $resultArray->[ $offset + 2 ];
            FRITZBOX_Log $hash, 5, "INFO: dect".$dectUser."_model: " . $resultArray->[ $offset + 2 ];
            push @roReadings, "dect".$dectUser."_fwVersion|" . $resultArray->[ $offset + 3 ];
            FRITZBOX_Log $hash, 5, "INFO: dect".$dectUser."_fwVersion: " . $resultArray->[ $offset + 3 ];
...
sub FRITZBOX_Readout_Add_Reading ($$$$@)
{
   ...
   if ($rName eq ",") {
     FRITZBOX_Log $hash, 1, "INFO: $rName: $rValue from caller -> $sub.$xline";
   }

   FRITZBOX_Log $hash, 5, "INFO: $rName: $rValue";
}
...
#         unless( defined ($result->{box_macFilter_active}) ) {
      ...
      FRITZBOX_Log $hash, 5, "INFO: set $name $cmd " . join(" ", @webCmdArray);
      ...
      FRITZBOX_Log $hash, 5, "INFO: get $name $cmd " . $queryStr;
   ...
   $returnStr = join('|', @roReadings );
   FRITZBOX_Log $hash, 5, "INFO: Handover to main process: " . $returnStr;
   ...
   if(defined $result->{Error}) {
      ...
      eval {
         foreach my $key (keys %$views) {
            FRITZBOX_Log $hash, 5, "INFO: Kid Profiles: ".$key;
      ...
      if ($ProfileOK eq "false") {
         ...
         FRITZBOX_Log $hash, 5, Dumper $lanDevice_Info;
...
sub FRITZBOX_Run_enableVPNshare($)
{
   ...
   if ( defined $result->{Error} ) {
     ...
     if ($vpnok == 0){
       ...
       FRITZBOX_Log $hash, 5, "DEBUG: data.lua: " . join(" ", @webCmdArray);
     ...
     if ( defined $result->{Error} ) {
       ...
       if ($lock_res =~ /ERROR/) {
         ...
         unless (($lock_res =~ /blocked/ && $val[1] eq "on") || ($lock_res =~ /unlimited/ && $val[1] eq "off")) {
           ...
           FRITZBOX_Log $hash, 3, "INFO: " . $lock_res . " -> $name $cmd $val[1]";
...
sub FRITZBOX_Call_Run_Shell($) 
{
 ...
 # Check if 2nd parameter is the duration
   ...
   if (int @val)
   {
      ...
      {
         $duration = $val[0];
         FRITZBOX_Log $hash, 5, "INFO: Extracted call duration of $duration s.";
         shift @val;
      }
   }
...
# Create tts link to say as moh
   if ( $field{say} ) 
   {
      ...
      {
         FRITZBOX_Log $hash, 2, "INFO: Cannot do Text2Speech because box has no music on hold";
      }
      else
      {
         chop $field{say};
         ...
         $ttsLink =~ s/\[TEXT\]/$ttsText/;
         FRITZBOX_Log $hash, 5, "INFO: Created Text2Speech internet link: $ttsLink";
      }
   }
   ...
   if ($field{play})
   {
      unless ($hash->{READINGS}{box_moh})
      {
         FRITZBOX_Log $hash, 2, "INFO: Cannot play mp3 because box has no music on hold";
      }
      elsif ($ttsLink)
      {
      ...
      else
      {
         chop $field{play};
         $ttsLink = $field{play};
         FRITZBOX_Log $hash, 5, "INFO: Extracted MP3 ring tone: $ttsLink";
      }
   }
...
# Create a hash for the DECT devices whose ring tone (or radio station) can be changed
   foreach ( split( /,/, $intNo ) ) {
      if (defined $hash->{fhem}{$_}{brand} && "AVM" eq $hash->{fhem}{$_}{brand}) {
         FRITZBOX_Log $hash, 5, "INFO: Internal number $_ seems to be a Fritz!Fon.";
 ...
 # Check if 2nd parameter is the duration
   ...
   if (int @val)
   {
      ...
      {
         $duration = $val[0];
         FRITZBOX_Log $hash, 5, "INFO: Extracted ring duration of $duration s.";
         shift @val;
      }
   }
 ...
 # Check if next parameter is a valid ring tone
   if (int @val)
   {
      if ($val[0] !~ /^(msg|show|say|play):/i)
      {
         ...
         FRITZBOX_Log $hash, 5, "INFO: Ring tone $ringTone will be used.";
         shift @val;
      }
   }
...
# Create tts link to play as internet radio
   ...
   if ($field{play})
   {
      ...
      {
        FRITZBOX_Log $hash, 2, "INFO: Cannot play mp3 because box has no internet radio";
      }
      elsif ($ttsLink)
      {
         FRITZBOX_Log $hash, 3, "INFO: Ignore 'play:' because Text2Speech already defined.";
...
# Create a hash for the DECT devices whose ring tone (or radio station) can be changed
   foreach ( split( /,/, $intNo ) ) {
      if (defined $hash->{fhem}{$_}{brand} && "AVM" eq $hash->{fhem}{$_}{brand}) {
         ...
         FRITZBOX_Log $hash, 5, "INFO: Internal number $_ (dect$userId) seems to be a Fritz!Fon.";
...
# Set ring tone Internet Radio
      FRITZBOX_Log $hash, 5, "INFO: Set ring tone of $intNo to radio $radioStation";
   ...
   FRITZBOX_Web_CmdPost( $hash, \@webCmdArray );
      ...
      FRITZBOX_Log $hash, 5, "INFO: Call $intNo";
...
# Reset ring tone
      FRITZBOX_Log $hash, 5, "INFO: Reset ring tones.";
...
sub FRITZBOX_Shell_Exec_Local($$)
{
   ...
   if (ref \$cmd eq "SCALAR") {
      FRITZBOX_Log $hash, 5, "INFO: Execute '".$cmd."'";
      my $result = qx($cmd);
      chomp $result;
      FRITZBOX_Log $hash, 5, "INFO: Result '$result'";
      return $result;
   }
      ...
      if ( int (@{$cmd}) > 0 )
      {
         ...
         FRITZBOX_Log $hash, 5, "INFO: Commands: '" . join( " | ", @{$cmd} ) . "'";
         my $cmdStr = join "\necho ' |#|'\n", @{$cmd};
         $cmdStr .= "\necho ' |#|'";
         my $result = qx($cmdStr);
         ...
         FRITZBOX_Log $hash, 5, "INFO: Result: '" . join (" | ", @resultArray)."'";
         return \@resultArray;
      }
      else
      {
...
sub FRITZBOX_Shell_Exec_Telnet($$)
{
   ...
   if (ref \$cmd eq "SCALAR") {
      ...
      if ( int (@{$cmd}) > 0 )
      {
         ...
         foreach (@{$cmd})
         {
            FRITZBOX_Log $hash, 5, "INFO: Execute '$_'";
            unless ($_ =~ /^sleep/)
            {
               @output=$telnet->cmd($_);
               $result = $output[0] || "";
               ...
               my $log = join "", @output;
               chomp $log;
               FRITZBOX_Log $hash, 5, "INFO: Result '$log'";
            }
            else
            {
...
sub FRITZBOX_Telnet_OpenCon($)
{
   ...
   unless (defined $pwd) {
      ...
      FRITZBOX_Log $hash, 5, "INFO: Open password file '$pwdFile' to extract password";
      if (open(IN, "<" . $pwdFile)) {
         $pwd = <IN>;
         close(IN);
        FRITZBOX_Log $hash, 5, "INFO: Close password file";
      } else {
         FRITZBOX_Log $hash, 2, $msg;
         return $msg;
      }
   ...
   FRITZBOX_Log $hash, 5, "INFO: Wait for user or password prompt.";
   ...
   if ( $match =~ /(user|login): / && $user eq "") {
      ...
      FRITZBOX_Log $hash, 5, "INFO: Entering user name";
      $telnet->print( $user );

      FRITZBOX_Log $hash, 5, "INFO: Wait for password prompt";
      ...
      FRITZBOX_Log $hash, 5, "INFO: Entering password";
   $telnet->print( $pwd );

      FRITZBOX_Log $hash, 5, "INFO: Wait for command prompt";
...
# redirect console messages
   $telnet->cmd("setconsole -r");
      ...
      FRITZBOX_Log $hash, 5, "INFO: Change command prompt";
...
# Set Password und User for TR064 access
   ...
   FRITZBOX_Log $hash, 3, "INFO: Credentials: " . $host . " / " . $FRITZBOX_TR064user . " / " . $FRITZBOX_TR064pwd;
...
sub FRITZBOX_TR064_Get_ServiceList($)
{
   ...
   FRITZBOX_Log $hash, 5, "INFO: Getting service page $url";
   my $agent    = LWP::UserAgent->new( env_proxy => 1, keep_alive => 1, protocols_allowed => ['http'], timeout => 10);
...
# Get actions of each service
   foreach (@serviceArray) {
      ...
      $url = 'http://'.$host.":49000".$_->[2];
         ...
         FRITZBOX_Log $hash, 5, "INFO: Getting action page $url";
      my $agent    = LWP::UserAgent->new( env_proxy => 1, keep_alive => 1, protocols_allowed => ['http'], timeout => 10);
      my $response = $agent->get( $url );
...
sub FRITZBOX_Web_CmdGet($$)
{
   ...
   foreach ( @{$getCmdArray} ) {
      ...
      if ($response->is_error) {
         ...
         FRITZBOX_Log $hash, 5, "INFO: Response: ".$response->content();
      }
   }
...
sub FRITZBOX_Web_Query($$@)
{
   ...
   FRITZBOX_Log $hash, 5, "INFO: Request data via API " . $f_lua;
   my $host = $hash->{HOST};
   my $url = 'http://' . $host;
   ...
   FRITZBOX_Log $hash, 5, "INFO: Response: " . $response->status_line . "\n" . $response->content;
...
sub FRITZBOX_Process_JSON($$$@) {
   ...
   if ($jsonText =~ /<html|"pid": "logout"|<head>/) {
      FRITZBOX_Log $hash, 5, "INFO: Old SID not valid anymore. ResetSID";
   ...
   FRITZBOX_Log $hash, 5, "INFO: Decode JSON string.";
   ...
   # FRITZBOX_Log $hash, 5, "INFO: JSON: " . Dumper($jsonResult);
   
   #Not a HASH reference at ./FHEM/72_FRITZBOX_2.pm line 4662.
...
sub FRITZBOX_Lan_Device_Info($$$) {
   ...
   FRITZBOX_Log $hash, 5, "INFO: get $name $action " . $queryStr;
   ...
   if (exists $result->{data}->{vars}) {
     ...
     if ($action eq "info") {
       ...
       FRITZBOX_Log $hash, 5, "INFO: MsgId: " . $jsonMsgId;
...
sub FRITZBOX_Lua_Kids($$@)
{
   ...
   FRITZBOX_Log $hash, 5, "INFO: Request data via API dataQuery.";
   my $host = $hash->{HOST};
   my $url = 'http://' . $host . '/internet/kids_userlist.lua?sid=' . $sid; # . '&' . $queryStr;

   FRITZBOX_Log $hash, 5, "INFO: URL: $url";
...
#################
  ...
  # 2018.03.19 18:43:28 3: FRITZBOX: get Fritzbox luaQuery settings/sip
   if ( ref ($jsonResult) ne "HASH" ) {
      chop $jsonText;
      FRITZBOX_Log $hash, 5, "INFO: no json string returned (" . $jsonText . ")";
      my %retHash = ("Error" => "no json string returned (" . $jsonText . ")", "ResetSID" => "1");
      return \%retHash;
   }
   $jsonResult->{sid} = $sid;
...
<ul>
   ...
   <ul>
      ...
      <li><a name="macFilter"></a>
         ...
         <br>
         Activates/deactivates the MAC Filter. Depends to "new WLAN Devices in the FritzBox.<br>
